...
#ifdef C2D_DIRECT_DISPLAY_LIB 
	archive << myFontWidth << myFontHeight
			<< myFontColour << myFontType;
#endif
...
{
	int lineIndex;
	for (lineIndex = myPages[0].firstLine; lineIndex < myPages[0].lastLine; ++lineIndex)
	{
		Line &line = myLines[lineIndex];
...
void UIText::MoveCursor()
{
	if( myCursor )
	{
		...
		int32 width, height;
		Line &line = myLines[lineIndex];
		myClonedEntity->MeasureString( myText.substr( line.firstChar, myCursorPosition - line.firstChar ),
			font, width, height );
			...
			if (formattingCode)
			{
				...
				if (tokenName == "tint")
				{
					...
					int font = -1;
						...
						FilePath path ( myFontName+".s16", IMAGES_DIR);
						path.LowerCase();
						font = myClonedEntity->SelectFontType( path.GetFullPath(), r, g, b, rot, swap);
					#endif
...
UITextPart::UITextPart(FilePath const& gallery,
					int baseimage,
					int numImages, 
					Vector2D& relPos,
					int relplane,
	...
	{ 
			...
			if(myClonedEntity)
			{
				myClonedEntity->SetFontAttributes( myFontWidth , 
										  myFontHeight, 
										  myFontColour, 
										  myFontType );
			}
...
UIGenerative::UIGenerative( FilePath const& gallery, int baseimage, int numImages,
	Vector2D& relPos, int relplane, int numValues ):
UIPartWithClonedImage(gallery, baseimage, numImages, relPos, relplane ),
myGenerationNumber(0),
myStartString(""),
...
std::string UIGenerative::GetStartString()
{
...
std::string UIGenerative::GetReplaceString()
{
	return myReplaceString;
}

int UIGenerative::GetGenerationNumber()
{
	return myGenerationNumber;
}

void UIGenerative::SetBranchLength(int length)
{
	myBranchLength = length;
}

int UIGenerative::GetBranchLength()
{
	return myBranchLength;
}
...
int UIGenerative::GetPerturbBranchLength()
{
	return myPerturbBranchLength;
}

void UIGenerative::SetPerturbDelta(float perturb)
{
	myPerturbDelta = perturb;	
}

float UIGenerative::GetPerturbDelta()
{
	return myPerturbDelta;
}

void UIGenerative::SetDelta(float delta)
{
	myDelta = delta;	
}

float UIGenerative::GetDelta()
{
	return myDelta;
}
