...
public:
  Port(AgentHandle &owner, const char *name, const char *desc,
       const Vector2D &relativePosition)
      : myOwner(owner), myName(name), myDescription(desc),
        myRelativePosition(relativePosition) {}

  Port(){};
  virtual ~Port() {}

  void Trash() { myOwner = NULLHANDLE; }
  const char *GetName() { return myName.c_str(); }
  void SetName(const char *name) { myName = name; }
  const char *GetDescription() { return myDescription.c_str(); }
  void SetDescription(const char *desc) { myDescription = desc; }
  AgentHandle &GetOwner() { return myOwner; }
  Vector2D GetRelativePosition() const { return myRelativePosition; }
