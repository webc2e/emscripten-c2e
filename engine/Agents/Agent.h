...
public:
  ...
  void YouAreNowDoomed() {
    _ASSERT(!myGarbaged);
    myImpendingDoom = true;
  }
  bool AreYouDoomed() {
    _ASSERT(!myGarbaged);
    return myImpendingDoom;
  }

  bool IsConnectionValid(Port *in, Port *out, bool &warning);
  ...
  uint32 GetAgentType() { return myAgentType; }

  // This function can be called on a garbaged agent
  int GetReferenceCount() { return myReferenceCount; }
  ...
  // This function can be called on a garbaged agent
  bool IsGarbage() { return myGarbaged; }
  ...
  inline void SetWhetherHighlighted(bool f, int r = 255, int g = 255,
                                    int b = 255) {
    _ASSERT(!myGarbaged);
    myHighlightedFlag = f;
  ...
  void SoundLevels(long &volume,
                   long &pan); // retn 1 if any part of agent is on screen
  virtual bool Visibility(int scope); // retn 1 if any part of agent is on
                                      // screen retn 2 if agent slightly off
                                      // screen retn 0 if agent is off screen
  virtual void Hide();
  virtual void Show();
  ...
  // ---------------------------------------------------------------------
  virtual Vector2D GetMapCoordOfWhereIPickUpAgentsFrom(int pose = -1,
  ...
  //				bounding box where I would like to attach to
  //agents 				carrying me.  (Default value is to be carried like a 				briefcase).
  // ---------------------------------------------------------------------
  ...
  //				agent by the mouse pointer which it is clicked
  //on.
  // ---------------------------------------------------------------------
  ...
  void GetClickActions(int &clac, int &clik1, int &clik2, int &clik3,
                       int &curclak) {
  ...
  virtual bool ShowPose(int pose,
                        int part = 0); // default helper fn for POSE macro
  ...
  Vector2D GetFloatingVector() { return myFloatingPositionVector; }
  ...
  enum {
    ...
    attrGreedyCabin =
        0x00000008, // 8 does vehicle autopickup things dropped in cabin
    attrInvisible =
        0x00000010, // 16 creatures don't see you - doesn't apply to ESEE or
                    // STAR, but only to internal creature code
...
protected:
  ...
  virtual void
  HandleDeactivate(Message *Msg);          // overloaded with valid fns in all
  ...
  virtual void HandleUI(Message *Msg); // message numbers between SCRIPTUIMIN &
                                       // SCRIPTUIMAX default calls HandleOther
  ...
  Classifier myClassifier; // Family/Genus/Species/00 identifier (used fr event
                           // macros & recognition)
  ...
  // myAgentType is used to store the type of the agent. This allows the
  // AgentHandles to do smart rtti which is faster than the slower C++ rtti
  // (Dan)
  uint32 myAgentType;
