...
// Filename:	Entity.cpp
// Class:		...
// Purpose:		taken from the header file (see the header for more
// details
//				written by Steve)
//
...
//invalidated. 	A library of pointers to all extant entities is stored in
//EntityLib[] 	for this purpose.
//
//	Entities are ALWAYS encapsulated in Objects of one kind or another.
//Thus, 	although each entity contains its own X&Y members, these get set as
...
// History:
// -------  Steve Grand		created
// 11Nov98	Alima			Started restructuring to separate the visual
// parts
//							of entity into a contained class
//that the display 							engine deals with.  The new contained class is 							called
//EntityImage.  The new member is myImage. 							I will not remove the current sprite
//system as I 							don't want to break the engine just yet.
//
// 04Dec98	Alima			Started removing all old display engine
// stuff
//							Objects now pass the name of the
//gallery 							containing their sprites to the new EntityImage
//
//							*********************************************
//							* Breaking Serialization until
//Entity Image *
//							* can serialize itself.	This
//should not be	*
//							* a problem since a blank world gets
//created*
//							* at the moment anyway.
//*
//							*********************************************
// 15Jan99	Alima removed Entity it has now been replaced by EntityImage in
//			 full.  That means EntityImage does that link thing with
//the 			worlds entityImagelib.
//
// --------------------------------------------------------------------------
...
// null constructor
BodyPart::BodyPart()
    : myAngle(0), myView(0), myOverlayShouldOverrideTheBaseBodySprite(0),
      myOverlay(0), myPart(0), myGenus(0), mySex(0), myVariant(0), myLastAge(0),
      myBodyPartType(0), myMirroredFlag(0) {
  // BaseImage is taken care of in myImage contsructor
}

BodyPart::BodyPart(int32 plane, int32 worldx, int32 worldy,
                   FilePath const &gallery, // gallery containing images
                   int32 numimages, uint32 baseimage, int part, int genus,
                   int sex, int age, int variant, int bodyPartType, bool mirror)
    : EntityImage(gallery, numimages, plane, worldx, worldy, baseimage,
                  baseimage),
      myAngle(0), myView(0), myOverlayShouldOverrideTheBaseBodySprite(0),
      myOverlay(0), myPart(part), myGenus(genus), mySex(sex),
      myVariant(variant), myLastAge(age)

{
  myBodyPartType = bodyPartType;
  myMirroredFlag = mirror;

  SetBodyPartFileName(myPart, myGenus, mySex, age, myVariant);
  CreateClothes(myPart, myGenus, mySex, age, myVariant);
...
void BodyPart::Reload(FilePath const &gallery, // gallery containing images
                      uint32 baseimage, int age) {
...
void BodyPart::SetBodyPartFileName(int part, int genus, int sex, int age,
                                   int variant) {
  uint32 fsp = ValidFsp(part, genus, sex, age, variant, "C16", IMAGES_DIR);
  if (fsp) {
...
bool BodyPart::Write(CreaturesArchive &archive) const {
  ...
  archive << myPart << myGenus << mySex << myVariant << myLastAge;
...
bool BodyPart::Read(CreaturesArchive &archive) {
  ...
  // success
  CreateClothes(myPart, myGenus, mySex, myLastAge, myVariant);
...
void BodyPart::SetViewAndImage(int Direction) {
  ...
  // sprites for right facing sprites for certain body part that it looks ok
  // for. We have a separate account of the actual view so that skeleton can get
  // the correct attachment data.
  ...
  if (myBodyPartType == PART_LHUMERUS || myBodyPartType == PART_LRADIUS ||
      ...
      myBodyPartType == PART_LEFT_EAR || myBodyPartType == PART_RIGHT_EAR ||
      !myMirroredFlag) {
    DrawMirrored(false);
...
Limb::Limb(Limb *nextlimb, // daughter limb
           LimbData *type, // type of limb
           FilePath const &gallery, uint32 baseimage, int32 numimages,
           int32 part,  // part number
           int32 genus, // Genus (NORN, GRENDEL, ETTIN SIDE)
           int32 sex,   // IDEAL Sex to look for (MALE/FEMALE)
           int32 age,   // IDEAL age & variant to look for
           int32 variant, int32 bodyPartType,
           bool mirror)
    :                   // image set
      BodyPart(0,       // plane
               0,       // worldx
               ...
               numimages, baseimage, part, genus, sex, age, variant,
               bodyPartType, mirror) {
  myNextLimb = nextlimb;
  if (type)
  ...
  CreateClothes(part, genus, sex, age, variant);
}
...
void Limb::Reload(
    ...
    LimbData *(*GetLimbDataPtr)(int,  // part number
                                ...
                                int), // type of limb
    FilePath const &gallery, int32 BodySprites[NUMPARTS], int32 numimages,
    int32 age) {

  myLimbData =
      *((*GetLimbDataPtr)(partNumber, myGenus, mySex, age,
                          myVariant)); // take personal copy of attachment table
                                       // but only if there is one given because
                                       // Heads like to create them themselves

  BodyPart::Reload(gallery, // gallery containing images
                   BodySprites[partNumber], age);

  partNumber++;

  if (myNextLimb) {
    myNextLimb->Reload(partNumber, GetLimbDataPtr, gallery, BodySprites,
                       numimages, age);
  }
}
...
Body::Body(BodyData *type, // BodyData struct for body type
           FilePath const &gallery, uint32 baseimage,
           ...
           int32 variant, int32 bodyPartType,
           bool mirror)
    : // abs plane for body image
      BodyPart(0, 0, 0, gallery, numimages, baseimage, part, genus, sex, age,
               variant, bodyPartType, mirror),
      myPregnancyStage(0) {
...
void Body::Reload(BodyData *type, // BodyData struct for body type
                  FilePath const &gallery, uint32 baseimage, int32 age) {
  myBodyData = *type;              // take personal copy of attachment table
  myCurrentBodyData = &myBodyData; // start off pointing at your own body data
...
void Body::SetViewAndImage(int direction) {
  ...
  if (myPregnancyStage > 0) {
    ...
    if (myMirroredFlag) {
      SetCurrentIndexFromBase((myPregnancyStage * setsize) +
                              myViewForMirroring);
    } else {
...
void Body::SetPregnancyStage(float progesteroneLevel) {
  myPregnancyStage = Map::FastFloatToInteger(
      progesteroneLevel * (float)(NUMBER_OF_PREGNANCY_SPRITES - 1));
}
