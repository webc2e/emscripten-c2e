...
void BodyPart::StoreCurrentOverlayState()
{
	myOverlay->StoreCurrentState();
}

void BodyPart::RestoreOverlayState()
{
	myOverlay->ReturnToStoredState(myView);
}
...
bool BodyPart::Read(CreaturesArchive &archive)
{
	...
	if(version >= 3)
	{
		...
		archive >> 	myOverlay;
		archive >> myBodyPartFileName;
		// This recaculates the filename FACE returns, in case
		// the computer the creature was imported from doesn't
		// have the body part files
		// Age 0 as FACE always returns the baby face
		SetBodyPartFileName(myPart, myGenus, mySex, 0, myVariant);
...
//*************************************************************************
	...
	if(myOverlay) 
	{
		myOverlay->UpdateOverlay(myView);
			
		bool yesOrNo= false;
		for(int i = 0; i < 4; i++)
		{
			yesOrNo = myOverlay->GetOverlay(i) == -1 ? true : false;
			if(!(yesOrNo && mirrored))
			{
				yesOrNo = false;
			}

			EntityImage::UpdateOverlayMirroring(i, yesOrNo );
