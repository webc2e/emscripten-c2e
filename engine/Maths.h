...
#define OUR_MIN min
#else
#define OUR_MAX std::max
#define OUR_MIN std::min
#endif

inline int FastFloatToInteger(float f) { return (int)f; }

//---------- general bounded arithmetic on bytes --------//
// These global, inline functions add & subtract float values without allowing
// them to overflow the range 0.0 to 1.0. For example, to add two floats use
// "result = BoundedAdd(x,y)"
...
inline unsigned int rand(void) {
  ...
  // return ( (( idum = 1664525 * idum + 1013904223 )>>16) )  &
  // (RandQD1::MAX_RAND);
...
inline int Rnd(int min, int max) {
  return ((((int)RandQD1::rand()) * (max - min + 1)) >>
          RandQD1::MAX_RAND_SHIFT) +
         min;
...
inline float RndFloat() { return ((float)Rnd(65536)) / 65535.0f; }

#ifdef _WIN32
// For some reason, Win32 isnan has an underscore
