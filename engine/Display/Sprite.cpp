		...
		if(myFileFormat == Gallery::IAmUncompressed)
		{
			for( i =0; i < NUM_BITMAPS; i++)
			{
				...
				if(myBitmaps[i]  && myBitmapsToDraw[i] == true)
				{
					...
					{
						DisplayEngine::theRenderer().
							DrawSprite(GetScreenPosition(),*myBitmaps[i]);
					}
				}
...
bool Sprite::SetBitmap(uint32 index, int32 layer,WhichGallery choice)
{
	...
	{
		if(!(myCompressedBitmaps[layer] = (CompressedBitmap*)galleryChoice->GetBitmap(index)))
			return false;
	}
return true;
}
...
uint32 Sprite::GetBitmapCount() const
{
	return myGallery->GetCount();
}
...
bool Sprite::IsPixelTransparent(uint32 x,
							uint32 y,
							int32 imageIndex)
{
...
//              			
// ----------------------------------------------------------------------
void Sprite::DrawOnMe(Position& whereToDraw,
...
void Sprite::DrawString( int32 x, int32 y, const std::string& text, int defaultFontIndex, std::vector<Sprite::CharStyle> charStyles, int stylesOffset )
{
...
int Sprite::SelectFont( const std::string &fontName, int r, int g, int b, int rot, int swap)
{
	FontCache::iterator it;
	int index = 0;
	for( it = myFontGalleries.begin(); it != myFontGalleries.end(); ++it )
	{
		if( it->myName == fontName &&
	...
	Gallery *gallery = SharedGallery::theSharedGallery().
		CreateClonedGallery(fontName, 0, 0);
	ASSERT(gallery);
...
void Sprite::MeasureString( const std::string &text,
								  int fontIndex,
...
void Sprite::DrawSpriteToBitmap( Bitmap* destination,
								 Position position,
								 Bitmap* const source )
{
...
void Sprite::CreateGalleryForCloning(std::string path,
								uint32 baseImage,
								uint32 pixelFormat,
								uint32 numBitmaps)
{
	...
	if(myGallery)
	{
		myGallery->SetUpGalleryShellForCloning(path,baseImage,pixelFormat,numBitmaps);
...
CreaturesArchive &operator<<( CreaturesArchive &ar, Sprite::CharStyle const &style )
{
