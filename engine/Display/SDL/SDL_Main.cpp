...
void HandleEvent(const SDL_Event &event) {
  static int btrans[3] = {InputEvent::mbLeft, InputEvent::mbMiddle,
                          InputEvent::mbRight};
  ...
  case SDL_MOUSEBUTTONUP:
    ...
    break;
  case SDL_KEYDOWN:
    theApp.GetInputManager().SysAddKeyDownEvent(event.key.keysym.scancode);
    break;
  case SDL_KEYUP:
    theApp.GetInputManager().SysAddKeyUpEvent(event.key.keysym.scancode);
    break;
