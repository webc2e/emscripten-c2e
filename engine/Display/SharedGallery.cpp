...
//				When a request for a new gallery comes in it
// creates 				a memorymapped file.  Then the new
// gallery is created 				whose bitmaps point to the
// relevent parts of the 				memory mapped file.
//
//				If an existing gallery is requested then the
// pointer for that 				gallery is passed back.
//
//
//				Note: at the moment there is no limit on how
// many
// files 				you can have open.  There is no
// reference checking to tell 				when to close galleries
// down
//
//
// History:
...
//
// ----------------------------------------------------------------------
SharedGallery &SharedGallery::theSharedGallery() { return mySharedGallery; }
...
//				creates one and adds it to the list of
// galleries.
// three 				different types of gallery can be
// created depending on 				the file extension.
// background, compressed, and normal gallery.
//
//				Now if you get an S16 extension check if there
// is a 				C16 version to use which will be
// preferred
// ----------------------------------------------------------------------
Gallery *SharedGallery::CreateGallery(std::string const &galleryIn) {
  ...
  if (x == -1) {
    if (testString.empty()) {
      ErrorMessageHandler::Throw(theDisplayErrorTag, (int)sidEmptyGalleryString,
                                 std::string("SharedGallery::DoCreateGallery"));
    } else {
      ErrorMessageHandler::Throw(theDisplayErrorTag, (int)sidNoGalleryFound,
                                 std::string("SharedGallery::DoCreateGallery"),
                                 testString.c_str());
    }
  ...
  if (it == myGalleries.end()) {
    ...
    gallery = DoCreateGallery(testString, 0, 0, false);
...
Gallery *SharedGallery::LookForCreatureGallery(std::string const &galleryName) {
  ...
  for (it = myCreatureGalleries.begin(); it != myCreatureGalleries.end();
       it++) {
...
Gallery *SharedGallery::CreateClonedGallery(std::string const &galleryIn,
  ...
  Gallery *gallery = DoCreateGallery(testString, baseImage, numImages,
                                     true); // clone
...
Gallery *SharedGallery::DoCreateGallery(std::string const &galleryIn,
                                        uint32 baseImage, uint32 numImages,
                                        bool clone) {
  std::string galleryName = galleryIn;
  ...
  if (ext == ".blk") { // not compressed background tile gallery
    tmpFilePath = galleryName + ".tmp.blk";
  } else if (ext == ".c16") { // compressed sprite gallery
  ...
  DisplayEngine::theRenderer().SafeImageConvert(
      galleryName, tmpFilePath, DisplayEngine::theRenderer().GetMyPixelFormat(),
      prevFileName);
  ::DeleteFile(tmpFilePath.c_str());

  Gallery *gallery = new Gallery(galleryName, baseImage, numImages, clone);

  // TODO: this is just a warning for physics stuff
  // do you still use this???
  /*	if(!gallery->ValidateBitmapSizes(MINIMUM_BITMAP_WIDTH,
     MINIMUM_BITMAP_HEIGHT))
          {
...
bool SharedGallery::RemoveGallery(Gallery *gallery) {
  ...
  if (RemoveClonedGallery(gallery))
    return true;
  ...
  if (found) {
    ...
    if (!(((*it).second)->IsUsed())) {
      ...
      OutputFormattedDebugString( "deleting gallery \"%s\": count now %d\n",
              it->second->GetName().GetFullPath().c_str(), myGalleries.size() -
1); #endif*/
      delete (*it).second;
      (*it).second = NULL;
...
bool SharedGallery::RemoveClonedGallery(Gallery *&gallery) {
  ...
  if (found) {
    /*
#ifdef _DEBUG
            OutputFormattedDebugString( "deleting cloned gallery \"%s\": count
now %d\n", gallery->GetName().GetFullPath().c_str(), myClonedGalleries.size() -
1); #endif*/
    delete (*it).first;
...
void SharedGallery::DestroyGalleries() {
  ...
  if (myClonedGalleries.size()) {
    ...
    for (it2 = myClonedGalleries.begin(); it2 != myClonedGalleries.end();
         it2++) {
      delete (*it2).first;
    }
  ...
  if (myCreatureGalleries.size()) {
    ...
    for (it3 = myCreatureGalleries.begin(); it3 != myCreatureGalleries.end();
         it3++) {
...
//						   composite creature gallery
// file. 				numSpritesInFile - yes
...
//
// ----------------------------------------------------------------------
Gallery *SharedGallery::CreateGallery(std::string moniker, uint32 uniqueID,
                                      uint32 ValidParts, uint8 Sex, uint8 Age,
  ...
  if (onePassOnly) {
    ...
    while (gallery == NULL && !bodyPartGallery->IsComplete()) {
      ...
      gallery = bodyPartGallery->AddCompressedCreature(
          moniker, uniqueID, ValidParts, Sex, Age, BodySprites, creatureParts,
          numSpritesInFile);
    }
  } else {
    // try having compressed Norns now
  ...
  if (gallery) {
    ...
    if (gallery->IsValid()) {
      ...
      ErrorMessageHandler::Throw(theDisplayErrorTag, (int)sidGalleryNotCreated,
                                 std::string("SharedGallery::CreateGallery"),
                                 name.c_str());
...
void SharedGallery::ConvertGallery(std::string const &galleryPath, uint32 to) {
  ...
  if (x == -1) {
    ErrorMessageHandler::Throw(theDisplayErrorTag, (int)sidGalleryNotFound,
...
void SharedGallery::DumpGalleries() {
  ...
  out << "\n\nmyClonedGalleries\n\n";
  for (iter2 = myClonedGalleries.begin(); iter2 != myClonedGalleries.end();
       ++iter2) {
    out << iter2->second << " " << iter2->first->GetName() << "\n";
  }

  out << "\n\nmyCreatureGalleries\n\n";
  for (iter3 = myCreatureGalleries.begin(); iter3 != myCreatureGalleries.end();
       ++iter3) {
