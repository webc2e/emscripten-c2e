...
// renderable
//				object needs to ask the display engine to draw
// it.
//
//
//
...
//				by all renderable objects.  To this end this
//class has 				private constructors and a static member
// function which 				gives access to the one and only
// DisplayEngine to all clients
//
//
//				Notes: must allow the world width to be set from
// outside
//
// History:
// -------  Chris Wylie		created
// 11Nov98	Alima			Added comments.
//							Added myPitch for non linear
//memory format
// 15Nov98					Now deals with windowed or full
// screen mode 16Dec98					Now draws dirty rects
//							Added sprite mirroring
//and compression
// 03Feb98					Added conversion between 16 bit
// pixel formats
// --------------------------------------------------------------------------
...
public:
  ...
  void DrawString(Bitmap *destination, std::string text, bool centred,
                  uint16 textColour, uint16 backgroundColour);

  void DrawStringToBackBuffer(int x, int y, std::string text, bool centred,
                              uint16 textColour, uint16 backgroundColour);
  ...
  void DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
                ...
                uint8_t stippleoff = 0, uint32 stipplestartAt = 0);
  ...
  inline void OffsetDrawTile(Position &position, Bitmap &bitmap, RECT &clip);
  ...
  bool CreateFastObject(EntityImage &entity, int32 plane);
  ...
  bool BlitToFrontBuffer(RECT &destination, IDirectDrawSurface4 *image,
                         RECT &source, bool transparencyAware);

  bool BlitFromPrimaryToMe(RECT &destination, IDirectDrawSurface4 *dest,
                           RECT &source, bool transparencyAware);

  bool BlitFromBackBufferToMe(RECT &destination, IDirectDrawSurface4 *dest,
                              RECT &BackgroundRect, bool transparencyAware);

  bool BlitToBackBuffer(RECT &destination, IDirectDrawSurface4 *image,
                        RECT &source, bool transparencyAware);

  HDC GetGDC(RECT &viewArea);
  void ReleaseGDC(HDC gdcHandle);

  LPDIRECTDRAWSURFACE4 CreateSurface(int32 width, int32 height,
...
inline uint32 DisplayEngine::GetPitch() { return myPitch; }
