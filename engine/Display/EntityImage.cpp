...
void EntityImage::SetFrameRate(const uint8_t rate)
{
	if (rate < 1)
		return;
	myFrameRate = rate;
...
void EntityImage::SetAnim( const uint8_t* newanim, int length )
{
...
bool EntityImage::ValidateAnim( const uint8_t* newanim, int length )
{
	ASSERT( length <= MaxAnimLength );
...
bool EntityImage::SetBaseAndCurrentIndex(uint32 baseimage)
{
	   ...
	   baseimage < myNumberOfImages)
	{
		...
		uint8_t testAnim[MaxAnimLength];
		for(int i=0; i < MaxAnimLength; i++)
			testAnim[i] = (uint8_t)(myAnim[i]);
