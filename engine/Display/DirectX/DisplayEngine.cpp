...
void DisplayEngine::ClearBuffers(void)
{
	...
	catch(...)
	{
		...
		theFlightRecorder.Log(FLIGHT_SHUTDOWN, "Posting WM_DESTROY from DisplayEngine.cpp?\n");
...
//		OutputDebugString(msg);
	...
	if(myFullScreenFlag)
		...
		if FAILED(err = myDirectDraw->SetCooperativeLevel( myWindow,
			DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWREBOOT| DDSCL_ALLOWMODEX | DISCL_BACKGROUND | DISCL_NOWINKEY))
		{
			char msg[_MAX_PATH];
			HRESULTtoCHAR(err,msg);
//			OutputDebugString(msg);
...
//				OutputDebugString(msg);
	...
	if (!Check16BitFormat( storedDisplayType,
						foldersToConvert))
	{
		// Bomb out!
		std::string s = theCatalogue.Get(theDisplayErrorTag, (int)sidDodgyPixelFormat1);
		::MessageBox(NULL, s.c_str(), theApp.GetGameName().c_str(), MB_OK|MB_ICONSTOP);
...
void DisplayEngine::CreateUserInterfaceGalleries(std::string const & transitionGallery)
{
...
{
	std::string msg = theCatalogue.Get(theDisplayErrorTag, (int)sidConvertingGraphics);
	msg += imagePath;
	...
	{
		ErrorMessageHandler::Show(theDisplayErrorTag,
							(int)sidNoImagesDirectory,
							std::string("DisplayEngine::DoConversion"));
	}
}

bool DisplayEngine::Check16BitFormat(uint32& storedDisplayType,
									 std::vector<std::string>& foldersToConvert)
{
	uint32 format=0;
...
void DisplayEngine::DrawToFrontBuffer()
{
	...
	if (myIsTheWorldRoundFlag || myDesiredWorldRoundness)
	{
		if(OpenBackBuffer())
		{
			EasterEgg::CunningBlit(GetBackBufferPtr(), 
						GetBackBufferPtr(), 
						mySurfaceArea.right,
...
bool DisplayEngine::ChangeDisplayMode(uint32 width, uint32 height, bool forceChange)
{
	...
	if(myFullScreenFlag && okToChange )
	{
		...
		if FAILED(myDirectDraw->SetCooperativeLevel(myWindow, 
				DISCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWREBOOT | DISCL_BACKGROUND |  DISCL_NOWINKEY))
				return false;
...
bool DisplayEngine::DoChangeScreenMode(bool toWindowedMode)
{
	...
	{	
		...
		SetWindowLong(myWindow,GWL_STYLE,WS_CAPTION|WS_VISIBLE|WS_SYSMENU|WS_THICKFRAME|WS_MINIMIZEBOX|WS_MAXIMIZEBOX);
...
//	OutputDebugString("get drawing parameters\n");
	...
	if (!compressedData_ptr)
		throw BasicException("DisplayEngine::DrawCompressedSprite doesn't have sprite");
								  ...
								  int width, int height)
{
	...
	if (width < mySurfaceArea.right)
	{
		...
		ddbltfx.dwSize=sizeof(DDBLTFX);
		ddbltfx.dwFillColor = 0;
		surfaceToDrawOn->Blt(&r,NULL,NULL,DDBLT_COLORFILL|DDBLT_WAIT,&ddbltfx);
	}
