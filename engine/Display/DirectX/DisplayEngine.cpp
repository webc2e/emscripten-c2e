...
// Class:		DisplayEngine
// Purpose:		This does all of the direct draw type stuff.  Any
// renderable
//				object needs to ask the display engine to draw it.
//This class 				can handle block bitmaps (background tiles) and sprites which 				have
//transparency.
//
//
//
...
//				by all renderable objects.  To this end this class
//has 				private constructors and a static member function which 				gives access to
//the one and only DisplayEngine to all clients
//
//				Most of the work gets done by:
//				DrawBitmap() - for background tile draw for
//example 				OffsetDrawBitmap() - draws part of a tile corresponding to a dirty
//									 rect.
...
//							Added myPitch for non linear memory
//format
// 15Nov98					Now deals with windowed or full screen
// mode 16Dec98					Now draws dirty rects
//							Added sprite mirroring and
//compression
// 03Feb98					Added conversion between 16 bit pixel
// formats
// --------------------------------------------------------------------------
#ifdef _MSC_VER
...
//
// Description:
//				Display modes that the current video card can display
//are 				discovered at the start and entered into a vector 				ourResolutions.  These
//can later be interrogated when the 				user attempts to change display modes
//
//				Only record display modes with 16 bit pixel
//format.
//
// ----------------------------------------------------------------------
...
DisplayEngine::DisplayEngine()
    : myCurrentOffScreenBufferPtr(0), myDirectDraw(0), myFrontBuffer(0),
      myHWBackBuffer(0), myBackBuffer(0), myPixelFormat(RGB_UNKNOWN),
      myClipper(0), myWaitingForMessageBoxFlag(0), myProgressBitmap(NULL),
      myProgressSurface(NULL), myTransitionGallery(NULL), mySpriteSurface(0),
...
DisplayEngine::DisplayEngine(uint32 flags)
    : myCurrentOffScreenBufferPtr(0), myDirectDraw(0), myFrontBuffer(0),
      myHWBackBuffer(0), myBackBuffer(0), myClipper(0), myProgressBitmap(NULL),
      myProgressSurface(0), myTransitionGallery(NULL), mySpriteSurface(0),
      myProgressBarHasBeenStarted(0) {
  myFlags = flags;
  myIsTheWorldRoundFlag = false;
  myDesiredWorldRoundness = false;
...
void DisplayEngine::ClearBuffers(void) {
  try {
    ...
    if SUCCEEDED (myFrontBuffer->GetSurfaceDesc(&desc)) {
      ...
      myFrontBuffer->Blt(&dest, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                         &ddbltfx);
    }
    if SUCCEEDED (myBackBuffer->GetSurfaceDesc(&desc)) {
      ...
      myBackBuffer->Blt(&dest, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                        &ddbltfx);
    }
  } catch (...) {
    ErrorMessageHandler::Show(theDisplayErrorTag,
                              (int)sidGeneralDirectDrawError,
    ...
    theFlightRecorder.Log(FLIGHT_SHUTDOWN,
                          "Posting WM_DESTROY from DisplayEngine.cpp?\n");
...
bool DisplayEngine::Start(uint32 &storedDisplayType,
  ...
  err = myDirectDrawInterface->QueryInterface(IID_IDirectDraw4,
                                              (void **)&myDirectDraw);
  ...
  myDirectDraw->EnumDisplayModes(NULL, NULL, &enumAll,
                                 DisplayEngine::EnumModesCallback);

  // handle full screen
  if (myFullScreenFlag) {

    if FAILED (err = myDirectDraw->SetCooperativeLevel(
                   myWindow, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN |
                                 DDSCL_ALLOWREBOOT | DDSCL_ALLOWMODEX |
                                 DISCL_BACKGROUND | DISCL_NOWINKEY)) {
      char msg[_MAX_PATH];
      HRESULTtoCHAR(err, msg);
      //			OutputDebugString(msg);
      allOK = false;
  ...
  if (!Check16BitFormat(storedDisplayType, foldersToConvert)) {
    // Bomb out!
    std::string s =
        theCatalogue.Get(theDisplayErrorTag, (int)sidDodgyPixelFormat1);
  ...
  CreateUserInterfaceGalleries(transitionGallery);

  myEngineRunningFlag = true;

  if (!allOK) {
    ErrorMessageHandler::Show(theDisplayErrorTag, (int)sidDirectDrawNotCreated,
                              std::string("StartDisplayEngine"));
  }
...
void DisplayEngine::CreateUserInterfaceGalleries(
  ...
  if (!FileExists(transitionGallery.c_str()))
    return;

  // if this doesn't get created it's no big deal.
  myTransitionGallery = new Gallery(transitionGallery);
...
void DisplayEngine::DoConversion(std::string &imagePath) {
  ...
  if (status_ok) {
    ...
    ErrorMessageHandler::Show(theDisplayErrorTag, (int)sidNoImagesDirectory,
...
bool DisplayEngine::Check16BitFormat(
  ...
  if (storedDisplayType != myPixelFormat) {
    ...
    storedDisplayType = myPixelFormat;
  }
...
bool DisplayEngine::StoreFileBeingConverted(std::string& fileName)
        ...
        return theRegistry.CreateValue(valueName,
                                                        fileName,
                                                  HKEY_LOCAL_MACHINE);
        }
...
bool DisplayEngine::FileBeingConverted(std::string& fileName)
        {
                ...
                std::string valueName("Conversion Data");
                if( theRegistry.CreateValue(valueName,
                                                                fileName,
                                                          HKEY_LOCAL_MACHINE))
                {
                        if(fileName != "")
...
void DisplayEngine::GetWindowRect(RECT &rect) {
  rect.top = ourWindowRect.top;
  rect.bottom = ourWindowRect.bottom;
  rect.left = ourWindowRect.left;
  rect.right = ourWindowRect.right;
}
...
//				surface.  Full screen mode uses an intermediate
//surface 				for smoother results.
//
// ----------------------------------------------------------------------
bool DisplayEngine::CreateFullscreenDisplaySurfaces(void) {
  ...
  if FAILED (myDirectDraw->CreateSurface(&surfaceDescription, &myFrontBuffer,
                                         NULL))
    return false;
  ...
  if FAILED (myDirectDraw->CreateSurface(&surfaceDescription, &myBackBuffer,
                                         NULL)) {
    ...
    if (myDirectDraw->CreateSurface(&surfaceDescription, &myBackBuffer, NULL) !=
        DD_OK) {
      //	uh oh!
      return false;
    }
...
bool DisplayEngine::CreateWindowedDisplaySurfaces(void) {
  ...
  if FAILED (myDirectDraw->CreateClipper(0, &myClipper, NULL))
    return false;
  if FAILED (myClipper->SetHWnd(0, myWindow))
    return false;
  if FAILED (myFrontBuffer->SetClipper(myClipper))
    return false;
...
void DisplayEngine::Update(Background *background,
  ...
  if (surfaceToDrawOn == NULL) {
    ...
    HRESULT err = surfaceToDrawOn->Lock(NULL, &desc, DDLOCK_WAIT, NULL);
  ...
  DrawGuttering(surfaceToDrawOn ? surfaceToDrawOn : myBackBuffer,
                background->GetPixelWidth(), background->GetPixelHeight());
...
//				to the backbuffer.  Then draws the back buffer to
//the 				front buffer
//
// ----------------------------------------------------------------------
void DisplayEngine::DrawToFrontBuffer() {
  ...
  if (myFullScreenFlag) {
    ...
    if (myFrontBuffer->Blt(&rect, myBackBuffer, &ourSurfaceArea, DDBLT_WAIT,
                           NULL) != DD_OK) {
      myFrontBuffer->Restore();
    }
  }
...
void DisplayEngine::FlipScreenVertically() {
  ...
  // on my machine this will let me create the surface in video
  // memory but then I cannot lock it
  if (myDirectDraw->CreateSurface(&surfaceDescription, &backgroundTop, NULL) ==
          DD_OK &&
      myDirectDraw->CreateSurface(&surfaceDescription, &backgroundBottom,
                                  NULL) == DD_OK) {

    backgroundTop->Blt(&topScreen, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                       NULL);
    backgroundBottom->Blt(&bottomScreen, NULL, NULL,
                          DDBLT_COLORFILL | DDBLT_WAIT, NULL);

    while (topScreen.bottom < ourSurfaceArea.bottom / 2) {
      err =
          myFrontBuffer->Blt(&topScreen, backgroundTop, NULL, DDBLT_WAIT, NULL);

      err = myFrontBuffer->Blt(&bottomScreen, backgroundBottom, NULL,
                               DDBLT_WAIT, NULL);
    ...
    while (topScreen.bottom < ourSurfaceArea.bottom) {
      err = myFrontBuffer->Blt(&topScreen, myBackBuffer, &topScreen, DDBLT_WAIT,
                               NULL);

      err = myFrontBuffer->Blt(&bottomScreen, myBackBuffer, &bottomScreen,
                               DDBLT_WAIT, NULL);
      topScreen.top++;
      topScreen.bottom++;
      bottomScreen.top--;
...
bool DisplayEngine::FlipScreenHorizontally() {
  ...
  // on my machine this will let me create the surface in video
  // memory but then I cannot lock it
  if (myDirectDraw->CreateSurface(&surfaceDescription, &backgroundLeft, NULL) ==
          DD_OK &&
      myDirectDraw->CreateSurface(&surfaceDescription, &backgroundRight,
                                  NULL) == DD_OK) {
    ok = true;

    backgroundLeft->Blt(&leftScreen, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                        NULL);
    // use left screen because they are the same size
    backgroundRight->Blt(&leftScreen, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                         NULL);

    while (leftScreen.right < screenArea.right / 2) {
      // draw lines at both ends of the screen to come together
      err = myFrontBuffer->Blt(&leftScreen, backgroundLeft, NULL, DDBLT_WAIT,
                               NULL);
      ...
      if (FAILED(err)) {
        ...
        err = myFrontBuffer->Blt(&rightScreen, backgroundRight, NULL,
                                 DDBLT_WAIT, NULL);
      }
    ...
    while (leftScreen.right < ourSurfaceArea.right) {
      if (FAILED(err)) {
        ...
        err = myFrontBuffer->Blt(&leftScreen, myBackBuffer, &backLeft,
                                 DDBLT_WAIT, NULL);
      }
      ...
      if (FAILED(err)) {
        ...
        err = myFrontBuffer->Blt(&rightScreen, myBackBuffer, &backRight,
                                 DDBLT_WAIT, NULL);
      }
...
bool DisplayEngine::SlideScreen() {
  ...
  if (myDirectDraw->CreateSurface(&surfaceDescription, &incomingBackground,
                                  NULL) == DD_OK &&
      myDirectDraw->CreateSurface(&surfaceDescription, &outgoingBackground,
                                  NULL) == DD_OK &&
      myDirectDraw->CreateSurface(&surfaceDescription, &temp, NULL) == DD_OK) {
    outgoingBackground->Blt(&frontSurface, myFrontBuffer, &screenArea,
                            DDBLT_WAIT, NULL);
    incomingBackground->Blt(&backSurface, myBackBuffer, &backSurface,
                            DDBLT_WAIT, NULL);
  }

  while (rightScreen.left > 0 || leftScreen.right < ourSurfaceArea.right) {
    err = temp->Blt(&leftScreen, incomingBackground, &leftScreen, DDBLT_WAIT,
                    NULL);
    ...
    err = temp->Blt(&rightScreen, outgoingBackground, &rightScreen, DDBLT_WAIT,
                    NULL);
...
void DisplayEngine::Burst() {
  ...
  myFrontBuffer->Blt(&spriteBound, mySpriteSurface, NULL,
                     DDBLT_WAIT | DDBLT_KEYSRC, NULL);

  while (spriteBound.right - spriteBound.left < ourSurfaceArea.right) {
    res = myFrontBuffer->Blt(&spriteBound, mySpriteSurface, NULL,
                             DDBLT_WAIT | DDBLT_KEYSRC, NULL);
  ...
  for (int times = 0; times < 5; times++) {
    res = myFrontBuffer->Blt(NULL, mySpriteSurface, &clip,
                             DDBLT_WAIT | DDBLT_KEYSRC, NULL);
...
bool DisplayEngine::ChangeDisplayMode(uint32 width, uint32 height,
  ...
  if (myFullScreenFlag && okToChange) {
    ...
    if FAILED (myDirectDraw->SetCooperativeLevel(
                   myWindow, DISCL_EXCLUSIVE | DDSCL_FULLSCREEN |
    ...
    SetWindowPos(myWindow,     // handle to window
                 ...
                 height + 22,  // height
                 /*SWP_FRAMECHANGED*/
                 SWP_NOSENDCHANGING | SWP_NOSIZE // window-positioning flags
    );
...
//				pMsg - buffer to store the text descritpion of the
//error
//
// Returns:     None
//
...
void DisplayEngine::HRESULTtoCHAR(HRESULT hErr, char *pMsg) {
  ...
  case DDERR_ALREADYINITIALIZED:
    sprintf(pMsg, "DDERR_ALREADYINITIALIZED");
    break;
  case DDERR_CANNOTATTACHSURFACE:
    sprintf(pMsg, "DDERR_CANNOTATTACHSURFACE");
    break;
  case DDERR_CANNOTDETACHSURFACE:
    sprintf(pMsg, "DDERR_CANNOTDETACHSURFACE");
    break;
  case DDERR_CURRENTLYNOTAVAIL:
    sprintf(pMsg, "DDERR_CURRENTLYNOTAVAIL");
    break;
  case DDERR_EXCEPTION:
    sprintf(pMsg, "DDERR_EXCEPTION");
    break;
  case DDERR_GENERIC:
    sprintf(pMsg, "DDERR_GENERIC");
    break;
  case DDERR_HEIGHTALIGN:
    sprintf(pMsg, "DDERR_HEIGHTALIGN");
    break;
  case DDERR_INCOMPATIBLEPRIMARY:
    sprintf(pMsg, "DDERR_INCOMPATIBLEPRIMARY");
    break;
  case DDERR_INVALIDCAPS:
    sprintf(pMsg, "DDERR_INVALIDCAPS");
    break;
  case DDERR_INVALIDCLIPLIST:
    sprintf(pMsg, "DDERR_INVALIDCLIPLIST");
    break;
  case DDERR_INVALIDMODE:
    sprintf(pMsg, "DDERR_INVALIDMODE");
    break;
  case DDERR_INVALIDOBJECT:
    sprintf(pMsg, "DDERR_INVALIDOBJECT");
    break;
  case DDERR_INVALIDPARAMS:
    sprintf(pMsg, "DDERR_INVALIDPARAMS");
    break;
  case DDERR_INVALIDPIXELFORMAT:
    sprintf(pMsg, "DDERR_INVALIDPIXELFORMAT");
    break;
  case DDERR_INVALIDRECT:
    sprintf(pMsg, "DDERR_INVALIDRECT");
    break;
  case DDERR_LOCKEDSURFACES:
    sprintf(pMsg, "DDERR_LOCKEDSURFACES");
    break;
  case DDERR_NO3D:
    sprintf(pMsg, "DDERR_NO3D");
    break;
  case DDERR_NOALPHAHW:
    sprintf(pMsg, "DDERR_NOALPHAHW");
    break;
  case DDERR_NOCLIPLIST:
    sprintf(pMsg, "DDERR_NOCLIPLIST");
    break;
  case DDERR_NOCOLORCONVHW:
    sprintf(pMsg, "DDERR_NOCOLORCONVHW");
    break;
  case DDERR_NOCOOPERATIVELEVELSET:
    sprintf(pMsg, "DDERR_NOCOOPERATIVELEVELSET");
    break;
  case DDERR_NOCOLORKEY:
    sprintf(pMsg, "DDERR_NOCOLORKEY");
    break;
  case DDERR_NOCOLORKEYHW:
    sprintf(pMsg, "DDERR_NOCOLORKEYHW");
    break;
  case DDERR_NODIRECTDRAWSUPPORT:
    sprintf(pMsg, "DDERR_NODIRECTDRAWSUPPORT");
    break;
  case DDERR_NOEXCLUSIVEMODE:
    sprintf(pMsg, "DDERR_NOEXCLUSIVEMODE");
    break;
  case DDERR_NOFLIPHW:
    sprintf(pMsg, "DDERR_NOFLIPHW");
    break;
  case DDERR_NOGDI:
    sprintf(pMsg, "DDERR_NOGDI");
    break;
  case DDERR_NOMIRRORHW:
    sprintf(pMsg, "DDERR_NOMIRRORHW");
    break;
  case DDERR_NOTFOUND:
    sprintf(pMsg, "DDERR_NOTFOUND");
    break;
  case DDERR_NOOVERLAYHW:
    sprintf(pMsg, "DDERR_NOOVERLAYHW");
    break;
  case DDERR_NORASTEROPHW:
    sprintf(pMsg, "DDERR_NORASTEROPHW");
    break;
  case DDERR_NOROTATIONHW:
    sprintf(pMsg, "DDERR_NOROTATIONHW");
    break;
  case DDERR_NOSTRETCHHW:
    sprintf(pMsg, "DDERR_NOSTRETCHHW");
    break;
  case DDERR_NOT4BITCOLOR:
    sprintf(pMsg, "DDERR_NOT4BITCOLOR");
    break;
  case DDERR_NOT4BITCOLORINDEX:
    sprintf(pMsg, "DDERR_NOT4BITCOLORINDEX");
    break;
  case DDERR_NOT8BITCOLOR:
    sprintf(pMsg, "DDERR_NOT8BITCOLOR");
    break;
  case DDERR_NOTEXTUREHW:
    sprintf(pMsg, "DDERR_NOTEXTUREHW");
    break;
  case DDERR_NOVSYNCHW:
    sprintf(pMsg, "DDERR_NOVSYNCHW");
    break;
  case DDERR_NOZBUFFERHW:
    sprintf(pMsg, "DDERR_NOZBUFFERHW");
    break;
  case DDERR_NOZOVERLAYHW:
    sprintf(pMsg, "DDERR_NOZOVERLAYHW");
    break;
  case DDERR_OUTOFCAPS:
    sprintf(pMsg, "DDERR_OUTOFCAPS");
    break;
  case DDERR_OUTOFMEMORY:
    sprintf(pMsg, "DDERR_OUTOFMEMORY");
    break;
  case DDERR_OUTOFVIDEOMEMORY:
    sprintf(pMsg, "DDERR_OUTOFVIDEOMEMORY");
    break;
  case DDERR_OVERLAYCANTCLIP:
    sprintf(pMsg, "DDERR_OVERLAYCANTCLIP");
    break;
  case DDERR_OVERLAYCOLORKEYONLYONEACTIVE:
    sprintf(pMsg, "DDERR_OVERLAYCOLORKEYONLYONEACTIVE");
    break;
  case DDERR_PALETTEBUSY:
    sprintf(pMsg, "DDERR_PALETTEBUSY");
    break;
  case DDERR_COLORKEYNOTSET:
    sprintf(pMsg, "DDERR_COLORKEYNOTSET");
    break;
  case DDERR_SURFACEALREADYATTACHED:
    sprintf(pMsg, "DDERR_SURFACEALREADYATTACHED");
    break;
  case DDERR_SURFACEALREADYDEPENDENT:
    sprintf(pMsg, "DDERR_SURFACEALREADYDEPENDENT");
    break;
  case DDERR_SURFACEBUSY:
    sprintf(pMsg, "DDERR_SURFACEBUSY");
    break;
  case DDERR_CANTLOCKSURFACE:
    sprintf(pMsg, "DDERR_CANTLOCKSURFACE");
    break;
  case DDERR_SURFACEISOBSCURED:
    sprintf(pMsg, "DDERR_SURFACEISOBSCURED");
    break;
  case DDERR_SURFACELOST:
    sprintf(pMsg, "DDERR_SURFACELOST");
    break;
  case DDERR_SURFACENOTATTACHED:
    sprintf(pMsg, "DDERR_SURFACENOTATTACHED");
    break;
  case DDERR_TOOBIGHEIGHT:
    sprintf(pMsg, "DDERR_TOOBIGHEIGHT");
    break;
  case DDERR_TOOBIGSIZE:
    sprintf(pMsg, "DDERR_TOOBIGSIZE");
    break;
  case DDERR_TOOBIGWIDTH:
    sprintf(pMsg, "DDERR_TOOBIGWIDTH");
    break;
  case DDERR_UNSUPPORTED:
    sprintf(pMsg, "DDERR_UNSUPPORTED");
    break;
  case DDERR_UNSUPPORTEDFORMAT:
    sprintf(pMsg, "DDERR_UNSUPPORTEDFORMAT");
    break;
  case DDERR_UNSUPPORTEDMASK:
    sprintf(pMsg, "DDERR_UNSUPPORTEDMASK");
    break;
  case DDERR_VERTICALBLANKINPROGRESS:
    sprintf(pMsg, "DDERR_VERTICALBLANKINPROGRESS");
    break;
  case DDERR_WASSTILLDRAWING:
    sprintf(pMsg, "DDERR_WASSTILLDRAWING");
    break;
  case DDERR_XALIGN:
    sprintf(pMsg, "DDERR_XALIGN");
    break;
  case DDERR_INVALIDDIRECTDRAWGUID:
    sprintf(pMsg, "DDERR_INVALIDDIRECTDRAWGUID");
    break;
  case DDERR_DIRECTDRAWALREADYCREATED:
    sprintf(pMsg, "DDERR_DIRECTDRAWALREADYCREATED");
    break;
  case DDERR_NODIRECTDRAWHW:
    sprintf(pMsg, "DDERR_NODIRECTDRAWHW");
    break;
  case DDERR_PRIMARYSURFACEALREADYEXISTS:
    sprintf(pMsg, "DDERR_PRIMARYSURFACEALREADYEXISTS");
    break;
  case DDERR_NOEMULATION:
    sprintf(pMsg, "DDERR_NOEMULATION");
    break;
  case DDERR_REGIONTOOSMALL:
    sprintf(pMsg, "DDERR_REGIONTOOSMALL");
    break;
  case DDERR_CLIPPERISUSINGHWND:
    sprintf(pMsg, "DDERR_CLIPPERISUSINGHWND");
    break;
  case DDERR_NOCLIPPERATTACHED:
    sprintf(pMsg, "DDERR_NOCLIPPERATTACHED");
    break;
  case DDERR_NOHWND:
    sprintf(pMsg, "DDERR_NOHWND");
    break;
  case DDERR_HWNDSUBCLASSED:
    sprintf(pMsg, "DDERR_HWNDSUBCLASSED");
    break;
  case DDERR_HWNDALREADYSET:
    sprintf(pMsg, "DDERR_HWNDALREADYSET");
    break;
  case DDERR_NOPALETTEATTACHED:
    sprintf(pMsg, "DDERR_NOPALETTEATTACHED");
    break;
  case DDERR_NOPALETTEHW:
    sprintf(pMsg, "DDERR_NOPALETTEHW");
    break;
  case DDERR_BLTFASTCANTCLIP:
    sprintf(pMsg, "DDERR_BLTFASTCANTCLIP");
    break;
  case DDERR_NOBLTHW:
    sprintf(pMsg, "DDERR_NOBLTHW");
    break;
  case DDERR_NODDROPSHW:
    sprintf(pMsg, "DDERR_NODDROPSHW");
    break;
  case DDERR_OVERLAYNOTVISIBLE:
    sprintf(pMsg, "DDERR_OVERLAYNOTVISIBLE");
    break;
  case DDERR_NOOVERLAYDEST:
    sprintf(pMsg, "DDERR_NOOVERLAYDEST");
    break;
  case DDERR_INVALIDPOSITION:
    sprintf(pMsg, "DDERR_INVALIDPOSITION");
    break;
  case DDERR_NOTAOVERLAYSURFACE:
    sprintf(pMsg, "DDERR_NOTAOVERLAYSURFACE");
    break;
  case DDERR_EXCLUSIVEMODEALREADYSET:
    sprintf(pMsg, "DDERR_EXCLUSIVEMODEALREADYSET");
    break;
  case DDERR_NOTFLIPPABLE:
    sprintf(pMsg, "DDERR_NOTFLIPPABLE");
    break;
  case DDERR_CANTDUPLICATE:
    sprintf(pMsg, "DDERR_CANTDUPLICATE");
    break;
  case DDERR_NOTLOCKED:
    sprintf(pMsg, "DDERR_NOTLOCKED");
    break;
  case DDERR_CANTCREATEDC:
    sprintf(pMsg, "DDERR_CANTCREATEDC");
    break;
  case DDERR_NODC:
    sprintf(pMsg, "DDERR_NODC");
    break;
  case DDERR_WRONGMODE:
    sprintf(pMsg, "DDERR_WRONGMODE");
    break;
  case DDERR_IMPLICITLYCREATED:
    sprintf(pMsg, "DDERR_IMPLICITLYCREATED");
    break;
  case DDERR_NOTPALETTIZED:
    sprintf(pMsg, "DDERR_NOTPALETTIZED");
    break;
  case DDERR_UNSUPPORTEDMODE:
    sprintf(pMsg, "DDERR_UNSUPPORTEDMODE");
    break;
  case DDERR_NOMIPMAPHW:
    sprintf(pMsg, "DDERR_NOMIPMAPHW");
    break;
  case DDERR_INVALIDSURFACETYPE:
    sprintf(pMsg, "DDERR_INVALIDSURFACETYPE");
    break;
  case DDERR_DCALREADYCREATED:
    sprintf(pMsg, "DDERR_DCALREADYCREATED");
    break;
  case DDERR_CANTPAGELOCK:
    sprintf(pMsg, "DDERR_CANTPAGELOCK");
    break;
  case DDERR_CANTPAGEUNLOCK:
    sprintf(pMsg, "DDERR_CANTPAGEUNLOCK");
    break;
  case DDERR_NOTPAGELOCKED:
    sprintf(pMsg, "DDERR_NOTPAGELOCKED");
    break;
  case DDERR_NOTINITIALIZED:
    sprintf(pMsg, "DDERR_NOTINITIALIZED");
    break;
  case E_NOINTERFACE:
    sprintf(pMsg, "E_NOINTERFACE");
    break;
  case E_POINTER:
    sprintf(pMsg, "E_POINTER");
    break;

    // Custom error messages.
    //		case C2ERR_NOPIXELDESC : sprintf(pMsg, "C2ERR_NOPIXELDESC");
    //break; 		case C2ERR_UNKNOWNS16FORMAT : sprintf(pMsg,
    //"C2ERR_UNKNOWNS16FORMAT"); break; 		case C2ERR_NOGRAPHICSDIR	:
    //sprintf(pMsg, "C2ERR_NOGRAPHICSDIR"); break; 		case C2ERR_MUSTCONVERTSPRITES
    //: sprintf(pMsg, "C2ERR_MUSTCONVERTSPRITES"); break; 		case
    //C2ERR_COULDNOTLOADSPRITE : sprintf(pMsg, "C2ERR_COULDNOTLOADSPRITE");
    //break;

  // Localise this if we ever use the output from HRESULTtoCHAR
  // for anything other than OutputDebugString
  default:
    sprintf(pMsg, "Unknown Error 0x%x", hErr);
    break;
  }
}
...
//				Transparency encoding.  this is the longest set of
//code 				because I have different routines for whether there is 				no clipping.
//right.bottom clipping. left/top clipping.
//
// ----------------------------------------------------------------------
void DisplayEngine::DrawCompressedSprite(Position &position,
  ...
  if (!compressedData_ptr)
    throw BasicException(
        "DisplayEngine::DrawCompressedSprite doesn't have sprite");
...
bool DisplayEngine::CreateProgressBar(Bitmap *bitmap) {
  ...
  myProgressSurface = CreateSurface(myProgressBitmap->GetWidth(),
                                    myProgressBitmap->GetHeight(), false);
  if (myProgressSurface == NULL)
    return false;
  ...
  surfaceDescription.dwSize = sizeof DDSURFACEDESC2;
  HRESULT res =
      myProgressSurface->Lock(NULL, &surfaceDescription, DDLOCK_WAIT, NULL);
...
bool DisplayEngine::RenderBitmapToFrontBuffer(Bitmap *bitmap) {
  ...
  IDirectDrawSurface4 *surface =
      CreateSurface(bitmap->GetWidth(), bitmap->GetHeight(), false);

  // This can happen on Win2K when the screen is locked, and autosave happens...
  if (surface == NULL)
    return false;
...
uint16 *DisplayEngine::OpenBackBuffer(void) {
  ...
  // resolution
  if (myBackBuffer) {
    HRESULT err = myBackBuffer->Lock(NULL, &desc, DDLOCK_WAIT, NULL);
    if (FAILED(err)) {
      char msg[_MAX_PATH];
      HRESULTtoCHAR(err, msg);
      //				OutputDebugString(msg);
    ...
    myPitch = desc.lPitch >> 1;
    myPitchForBackgroundTiles = (myPitch - 128) * 2;
    ASSERT(myCurrentOffScreenBufferPtr);
...
void DisplayEngine::DrawGuttering(IDirectDrawSurface4 *surfaceToDrawOn,
  ...
  if (width < mySurfaceArea.right) {
    r.left = width;
    r.top = 0;
    r.right = mySurfaceArea.right;
    r.bottom = mySurfaceArea.bottom;

    ZeroMemory(&ddbltfx, sizeof(DDBLTFX));
    ddbltfx.dwSize = sizeof(DDBLTFX);
    ddbltfx.dwFillColor = 0;
    surfaceToDrawOn->Blt(&r, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                         &ddbltfx);
  }
