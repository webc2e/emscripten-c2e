...
// bitmap in the gallery 				which always points to
// the relevant tile.
//
// History:
// -------  Chris Wylie		Created
...
//on a cd
// drive then 							the original
// path remains assuming that the
...
Background::Background()
    : myWidth(0), myHeight(0), myGallery(NULL), myPixelWidth(0),
      myPixelHeight(0), myOwnerCamera(NULL) {}
...
bool Background::Create(FilePath const &gallery_name,
  ...
  if (!gallery_name.empty()) {
    ...
    myGallery =
        (BackgroundGallery *)SharedGallery::theSharedGallery().CreateGallery(
            name);
    //	ASSERT(myGallery);
...
//				drive (there may be more than one cd drive) we
// have
// the 				file on and update the given path.  The path is
// only
...
bool Background::ReadFromCD(std::string &galleryName) {
  ...
  const uint8_t DriveNameLength = 4;
...
//				we know which portions of the background we need
// to
// draw. 				Get the real world coordinates and then
// convert them to 				absolute coordinates from 0,0 by
// subtracting them from the 				top left corner.
//
//
// ----------------------------------------------------------------------
...
void Background::Draw(bool completeRedraw, std::vector<RECT> &dirtyRects,
  ...
  if (completeRedraw || dirtyRects.size() > 500) {
    ...
    /*		std::vector<RECT>::iterator rect;
                    ...
                    for(rect = dirtyRects.begin(); rect != dirtyRects.end();
       rect++)
                    {

                            top = ((*rect).top -
       myTopLeftWorldCoordinate.GetY())>>7; bottom = ((*rect).bottom -
       myTopLeftWorldCoordinate.GetY())>>7; left =
       ((*rect).left-myTopLeftWorldCoordinate.GetX())>>7; right =
       ((*rect).right-myTopLeftWorldCoordinate.GetX())>>7;
                    ...
                    for (y=top;y<=bottom; y++,ypos = ((y-y_div.quot) << 7) -
       y_div.rem)
                            {

                                    for (x=left;x<=right; x++,  xpos = ((x -
       x_div.quot) << 7) - x_div.rem)
                                    {
                                            ...
                                            bitmap =
       myGallery->GetTile((imod*myHeight) + y); if(bitmap)
                                             {

                                                     Position pos(xpos,ypos);
                                                    bitmap->SetPosition(Position(xpos,ypos));
                                            //	 bitmap->Draw();
                                            //	if((pos.GetX() < 0 || pos.GetY()
       < 0)||
                                            //		(pos.GetX() +
       DEFAULT_ENVIRONMENT_RESOLUTION > displayWidth ||
                                            //		pos.GetY()+
       DEFAULT_ENVIRONMENT_RESOLUTION > displayHeight)) bitmap->Draw();
                                            //	else
                                            //
       bitmap->DrawWholeBitmapRegardless();
...
Position &Background::GetDisplayPosition(void) { return myWorldPosition; }

int32 Background::GetTop() { return myTopLeftWorldCoordinate.GetY(); }

int32 Background::GetLeft() { return myTopLeftWorldCoordinate.GetX(); }

Position Background::GetTopLeft() { return myTopLeftWorldCoordinate; }
