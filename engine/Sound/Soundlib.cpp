...
//						  Playing from the "munged" music
//file 						  Commented out DSBSTATIC flag in PlaySound
...
bool CachedSound::Write(CreaturesArchive &archive) const { return false; }

bool CachedSound::Read(CreaturesArchive &archive) { return false; }

ActiveSample::ActiveSample()
    : pSample(NULL), wID(0), cloned(NULL), fade_rate(0), locked(FALSE),
      volume(0) {}
...
SoundManager::SoundManager()
    : maximum_size(0), current_size(0), last_used(0), sound_initialised(FALSE),
      overall_volume(0), target_volume(0), current_volume(0), myMidiModule(0),
      faded(false) {
  ...
  if (references == 1) {
    ...
    if (hr = DirectSoundCreate(NULL, &pDSObject, NULL) != DS_OK) {
      ...
      std::string string = ErrorMessageHandler::Format(
          "sound_error", (int)SoundManager::sidFailedToCreateDirectSoundObject,
    ...
    if ((hr = pDSObject->SetCooperativeLevel(hWnd, DSSCL_PRIORITY)) != DS_OK) {
      ...
      std::string string = ErrorMessageHandler::Format(
          "sound_error", (int)SoundManager::sidFailedToSetCooperativeLevel,
          std::string("SoundManager::SoundManager"));
      throw(SoundManager::SoundException(string, __LINE__));
    }
    ...
    hr = pDSObject->CreateSoundBuffer(&dsbdesc, &pPrimary, NULL);

    if (hr != DS_OK) {
      // Primary buffer not created
      std::string string = ErrorMessageHandler::Format(
          "sound_error", (int)SoundManager::sidPrimaryBufferNotCreated,
          std::string("SoundManager::SoundManager"));
      throw(SoundManager::SoundException(string, __LINE__));
    }
  ...
  ///////////////////////////////////////////////////
  // what we could also do here is to set up the PRIMARY buffer and play it -
  // this would reduce the startup time when the first secondary buffer is
  // played.
...
CachedSound *SoundManager::OpenSound(DWORD wave) {
  ...
  dsbdesc.dwFlags = DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPAN |
                    DSBCAPS_GLOBALFOCUS; //|DSBCAPS_STATIC;		//need
                                         //default controls (vol, pan, freq)
  ...
  hr = pDSObject->CreateSoundBuffer(&dsbdesc, &buffer, NULL);
  ...
  if (hr == DS_OK) {
    ...
    hr = buffer->Lock(0, numbytes, &lpvAudio1, &Bytes1, &lpvAudio2, &Bytes2, 0);

    // if we got a DSERR_BUFFERLOST try again
    if (hr == DSERR_BUFFERLOST) {
      buffer->Restore();
      hr = buffer->Lock(0, numbytes, &lpvAudio1, &Bytes1, &lpvAudio2, &Bytes2,
                        0);
    }
    ...
    ErrorMessageHandler::Show("sound_error", catalogue_index,
...
SOUNDHANDLE SoundManager::PlayCachedSound(CachedSound *wave, int volume,
                                          int pan, BOOL loop) {
...
SOUNDERROR
SoundManager::InitializeCache(int size) //	Set size (K) of cache in bytes
...
CachedSound *
SoundManager::EnsureInCache(DWORD wave) //  Places wave in cache and returns
{
...
SOUNDERROR SoundManager::PreLoadSound(DWORD wave) //  Ensures a sound is loaded
                                                  //  into the cache
{
  if (mixer_suspended || !sound_initialised) {
    return (SOUND_MIXER_SUSPENDED);
...
SOUNDERROR
SoundManager::PlaySoundEffect(DWORD wave, int ticks, long volume,
                              long pan) //  Loads in sound or locates within
...
SOUNDERROR SoundManager::StopControlledSound(SOUNDHANDLE handle, BOOL fade) {
  ...
  if (fade) {
    ...
    active_sounds[handle].fade_rate =
        (SoundMinVolume - active_sounds[handle].volume) / 15;
