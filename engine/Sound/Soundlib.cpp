...
#include "../Display/DirectX/Window.h" // window handle
#include "../FilePath.h"
...
// 
//		   Alima			Added midi module using direct music as an
//							alternative to using the generative music system.
// 30Jun00 Alima			Added a CDRom device controller and a sampler 
//							providing frequency spectrum data.
//			
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Method:		RestoreSoundManager
// Arguments:	None
// Description:	Restore all direct sound buffers.  Consult the app to 
//				choose between generative music and midi.
// ----------------------------------------------------------------------
void SoundManager::RestoreSoundManager()
{
	mungeFile = theApp.GetDefaultMNG();
	...
	if (references == 1)
		...
		if(hr = DirectSoundCreate(NULL, &pDSObject, NULL)!=DS_OK) {
			...
			std::string string = ErrorMessageHandler::Format("sound_error",
										(int)SoundManager::sidFriendlyFailedToCreateDirectSoundObject,
	...
	if(theApp.DoYouOnlyPlayMidiMusic())
	{
		if(!myMidiModule)
		{
			myMidiModule = new MidiModule(pDSObject, theMainWindow);
...
// ----------------------------------------------------------------------
// Method:		ReleaseDirectSoundObject
// Description:	Release all direct sound buffers useful if you want to 
//				borrow the mixer for some time.  
// ----------------------------------------------------------------------
void SoundManager::ReleaseDirectSoundObject()
{
	...
	// Close Down ALL direct Sound Activity
...
CachedSound *SoundManager::OpenSound(DWORD wave)
{
	...
	if (munged)
		...
		try
		{
			FilePath filePath(mungeFile, SOUNDS_DIR);
			std::string path = filePath.GetFullPath();

			file.Open(path,GENERIC_READ);	// SPARKY default is rw
	...
	else
	{
		try
		{
			...
			file.Open(wavFile,GENERIC_READ);	// SPARKY deault is rw
		}
		catch(File::FileException& e)
		{
			ErrorMessageHandler::Show(e, std::string("SoundLib::OpenSound"));
				...
				int volume, int pan, BOOL loop)
{
	...
	if(result==DS_OK)
	{
		...
		if (volume < SoundAbsoluteMinVolume)
			{
			volume = SoundAbsoluteMinVolume;
			}

		pDSBuffer->SetVolume(volume);

		if (volume > 0 || volume < SoundAbsoluteMinVolume)
			{
...
int SoundManager::GetWaveSize(DWORD wave)
{
	...
	if (wave >= 0xff000000)
		...
		try
		{
			FilePath filePath(mungeFile, SOUNDS_DIR);
			std::string path = filePath.GetFullPath();
			munged.Open(path,GENERIC_READ);	// SPARKY, default was read/write
	...
	else
	{
		...
		try
		{
			...
			file.Open(wavFile,GENERIC_READ);	// SPARKY was rw
		}
		catch(File::FileException& e)
		{
			ErrorMessageHandler::Show(e, std::string("SoundLib::GetWaveSize"));
...
SOUNDERROR SoundManager::FadeOut()
	...
	target_volume = SoundAbsoluteMinVolume;
...
int SoundManager::GetVolumeOnMidiPlayer()
{
	if(myMidiModule)
	{
		return myMidiModule->GetVolume();
	}
	return 100;
}
...
{
	// stop all access to the sound card
	ReleaseDirectSoundObject();
}
