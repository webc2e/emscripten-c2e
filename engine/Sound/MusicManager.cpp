...
{
	desiredTrack = "";
	currentMNG = theApp.GetDefaultMNG();
}
...
// Description:	Loads and compiles script from Sounds/*.mng files
// ----------------------------------------------------------------------
bool MusicManager::LoadScrambled()
	...
	try
	{
		FilePath filePath(currentMNG, SOUNDS_DIR);
...
MusicError MusicManager::Parse(MusicScript &script)
	...
	while (script.GetCurrentType() != MusicScript::EndOfFile)
		...
		if (strcmp( script.GetCurrentToken(), "Track") == 0)
			...
			MusicTrack *track = GetTrack ( trackName.c_str() );
...
void MusicManager::Update()
	...
	switch ( trackEvent )
		...
		case ( BeginEvent ):
			{
			ActivateBeginTrack(nextTrackName.c_str());
			break;
			}
		case ( InteruptEvent ):
			{
			ActivateInteruptTrack(nextTrackName.c_str());
			break;
			}
		case ( FadeEvent ):
			{
...
void MusicManager::ActivateBeginTrack(LPCTSTR name)
	...
	if ( incomingMNG.find('\\') != -1 )
		...
		incomingMNG = theApp.GetDefaultMNG();
