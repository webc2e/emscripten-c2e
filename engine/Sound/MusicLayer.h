...
//					Added list of controlled sounds (allows proper
//fading)
...
public:
  ...
  // ----------------------------------------------------------------------
  virtual MusicError Parse(MusicScript &script, MusicManager &manager,
                           MusicTrack &track);

  // ----------------------------------------------------------------------
  // Method:		Update
  // Arguments:	uberVolume  - overall soundtrack volume
  //				time		- current time (seconds elapsed
  //since 							  start of track) 				beat		- current length of a beat in seconds
  //				beatCount   - total beats elapsed
  // Returns:		Nothing
  // Description:	Updates the layer, taking into account overall volume
  // ----------------------------------------------------------------------
  virtual void Update(MusicValue uberVolume, MusicValue time, MusicValue beat,
  ...
  //				The wave has not already been encountered, it will
...
protected:
  ...
  // Description:	Temporary helper function, left in until the
  // soundmanager
  ...
  // Returns:		Volume in SoundManager scale (ranges from -10000 to
  // 10000)
  // ----------------------------------------------------------------------
  ...
  //				vol			- volume of sound (0.0
  //to 1.0) 				effect		- Id of effect from shared library
  // Returns:		Nothing
  ...
  //				wave		- Id of wave to be played
  //				vol			- volume of sound (0.0
  //to 1.0) 				pan			- stereo position (-1.0 to 1.0)
  ...
  // ----------------------------------------------------------------------
  SoundHandle LoopSound(MusicValue uberVolume, MusicWaveID wave, MusicValue vol,
                        MusicValue pan);
  ...
  // ----------------------------------------------------------------------
  void UpdateLoop(MusicValue uberVolume, SoundHandle handle, MusicValue vol,
                  MusicValue pan);
  ...
  //				intendedVolume - volume of the sound, before the
  //layer 								 volume has been taken into account 				initialPan	   - initial pan setting
  // Returns:		Nothing
  // Description:	Adds a new controlled sound to the list
  // ----------------------------------------------------------------------
  void AddControlledSound(SoundHandle soundHandle, MusicValue intendedVolume,
...
private:
  ...
  // ----------------------------------------------------------------------
  void QueueSound(MusicWaveID wave, MusicValue vol, MusicValue pan,
                  MusicValue delay);
  ...
  public:
    ...
    // ----------------------------------------------------------------------
    QueuedSound(MusicWaveID wave, MusicValue vol, MusicValue pan,
                MusicValue delay)
        : id(wave), volume(vol), panning(pan), playTime(delay) {}
    ...
    bool IsReady(MusicValue time) {
      return (time + musicTimerResolution) >= playTime;
    }
  ...
  public:
    ...
    // Arguments:	soundHandle    - handle of the new sound
    //				intendedVolume - volume of the sound, before the
    //layer 								 volume has been taken into account 				initialPan	   - initial pan
    //setting
    // Returns:		Nothing
    // Description:	Constructor
    // ----------------------------------------------------------------------
    ControlledSound(SoundHandle soundHandle, MusicValue intendedVolume,
                    MusicValue initialPan);
    ...
    BOOL IsFinished() {
      return theMusicSoundManager
                 ? theMusicSoundManager->FinishedControlledSound(handle)
                 : TRUE;
    }
  ...
  //	typedef std::set<ControlledSound *>::reverse_iterator
  //controlledReverseIterator;
