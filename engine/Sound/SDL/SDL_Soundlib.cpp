...
//  Is mixer running?
bool SoundManager::SoundEnabled()
{
	return true;
}
...
SOUNDERROR SoundManager::SetVolume(long volume)
{
	...
	for( i=0; i<MAX_ACTIVE_SOUNDS; ++i )
	{
		if( myActiveSounds[i].wave )
		{
			...
			// std::cout << "2 Mix SDL volume (SetVolume)" << (currentvol * volume)/128 << std::endl;
...
SOUNDERROR SoundManager::PlaySoundEffect(DWORD wave, int ticks,
...
SOUNDERROR SoundManager::StartControlledSound(DWORD wave,
	SOUNDHANDLE &handle, long volume, long pan, BOOL looped)
{
	if( !myInitFlag )
		return NO_SOUND_ERROR;
...
SOUNDERROR SoundManager::UpdateControlledSound(SOUNDHANDLE handle,
	long volume, long pan)
{
	int i=(int)handle;
	...
	volume = ToSDLVolume( volume );
	// std::cout << "3 Mix SDL volume (UpdateControlledSound)" << (GetOverallFaded() * volume)/128 << std::endl;
...
SOUNDERROR SoundManager::StopControlledSound(SOUNDHANDLE handle,
	BOOL fade)
{
	if( !myInitFlag )
		return NO_SOUND_ERROR;
...
void SoundManager::SetMNGFile(const std::string& mng)
{
	...
	if( !myMungedFP )
	{
		throw SoundException( "SoundManager::SetMNGFile() failed",
			__LINE__ );
	}
}


bool SoundManager::IsMixerFaded()
{
	return myFaded;
}
...
// convert game (dsound?) style volume scale (-10000...0) to the SDL scale (0..128)
...
void SoundManager::StartSoundOneShot( SoundManager::CachedSound* s,
	int vol, int pan )
{
	// find a free slot
	int i=0;
	...
	if( Mix_PlayChannel( myActiveSounds[i].channel,
		s->chunk, 0 ) < 0 )
	{
		return;
	}
	...
	// std::cout << "4 Mix SDL volume (StartSoundOneShot)" << playvol << std::endl;
...
int SoundManager::StartSoundControlled( SoundManager::CachedSound* s,
	int vol, int pan, BOOL looping )
{
	// find a free slot
	int i=0;
	while( i< MAX_ACTIVE_SOUNDS )
...
// std::cout << "Mix raw volume " << vol << std::endl;
	int playvol = (ToSDLVolume(vol) * GetOverallFaded())/128;
    // std::cout << "5 Mix SDL volume (StartSoundControlled)" << playvol << std::endl;
	Mix_Volume( myActiveSounds[i].channel, playvol );
...
Mix_Chunk* SoundManager::LoadWAVFromMunged( DWORD wave )
{
	...
	*p++ = ' ';
	...
	Mix_Chunk *mixChunk = Mix_LoadWAV_RW( SDL_RWFromMem( myMungeBuf, myMungeBufSize ), 1 );
	if (!mixChunk)
...
CDRomDevice& SoundManager::GetCDRomDevice()
{
	return myStubCDRom;
}
...
{
	return myUnconvertedOverallVolume;
}

int SoundManager::GetOverallFaded()
{
	return myFaded ? 0 : myOverallVolume;
}
