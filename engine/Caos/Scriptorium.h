...
//				This class is heavily used every time the game tries
//to 				execute an event, or trigger a script.
//
...
public:
  ...
  //				if a script with the same classifier already
  //exists, 				it will be replaced, and the old script will be 				deleted. 				However, if
  //the existing script is still in use (ie 				locked), this function will fail.
  // ---------------------------------------------------------------------
  ...
  //				NOTE - if no matching script is found, this
  ...
  //				Unlike FindScript(), there is no falling back to
  //more 				general scripts.
  // ---------------------------------------------------------------------
  MacroScript *FindScriptExact(const Classifier &c);
