...
int RuntimeErrorDialog::DisplayDialog(HWND wnd) {
  ...
  int ret = DialogBox(NULL, MAKEINTRESOURCE(IDD_RUNTIME_ERROR_DIALOG), wnd,
                      (DLGPROC)RuntimeErrorDlgProc);
...
LRESULT CALLBACK RuntimeErrorDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam,
                                     LPARAM lParam) {
  ASSERT(currentRuntimeErrorDialog);
  return currentRuntimeErrorDialog->ProcessMessage(hDlg, uMsg, wParam, lParam);
}

LRESULT RuntimeErrorDialog::ProcessMessage(HWND hDlg, UINT uMsg, WPARAM wParam,
                                           LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
    SetWindowText(hDlg, theCatalogue.Get("system_title", 0));
    SetWindowText(GetDlgItem(hDlg, ID_IGNORE),
                  theCatalogue.Get("runtime_error", 0));
    SetWindowText(GetDlgItem(hDlg, ID_FREEZE),
                  theCatalogue.Get("runtime_error", 1));
    SetWindowText(GetDlgItem(hDlg, ID_KILL),
                  theCatalogue.Get("runtime_error", 2));

    SetWindowText(GetDlgItem(hDlg, ID_CAMERA_TO),
                  theCatalogue.Get("runtime_error", 3));
    SendMessage(GetDlgItem(hDlg, ID_CAMERA_TO), BM_SETCHECK,
                myCameraChecked ? BST_CHECKED : BST_UNCHECKED, 0);
    EnableWindow(GetDlgItem(hDlg, ID_CAMERA_TO), myCameraPossible);

    SetWindowText(GetDlgItem(hDlg, IDC_EDIT_CONTROL), myText.c_str());
    SetWindowText(GetDlgItem(hDlg, ID_PAUSE_GAME),
                  theCatalogue.Get("runtime_error", 6));
    SetWindowText(GetDlgItem(hDlg, ID_STOP_SCRIPT),
                  theCatalogue.Get("runtime_error", 8));
    return true;
  }
  case WM_COMMAND: {
...
//                                      LPARAM lParam) {
//   ASSERT(currentRuntimeErrorDialog);
//   return currentRuntimeErrorDialog->ProcessMessage(hDlg, uMsg, wParam,
//   lParam);
// }
