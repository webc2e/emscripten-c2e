...
void AgentHandlers::SubCommand_NEW_SIMP(CAOSMachine &vm) {
  ...
  AgentHandle agent(theAgentManager.CreateSimpleAgent(
      family, genus, species, FilePath(gallery + ".s16", IMAGES_DIR),
...
void AgentHandlers::SubCommand_NEW_CREA(CAOSMachine &vm) {
  ...
  agent =
      theAgentManager.CreateCreature(family, gene, gene_index, sex, variant);
...
void AgentHandlers::Command_GALL(CAOSMachine &vm) {
  ...
  if (found) {
    vm.GetTarg().GetAgentReference().SetGallery(
        FilePath(filename + ".s16", IMAGES_DIR), baseindex, vm.GetPart());
  }
}
