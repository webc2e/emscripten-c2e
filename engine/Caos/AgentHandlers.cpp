...
{
	vm.ValidateTarg();
	return vm.GetTarg().GetAgentReference().GetPresence().GetViewArcSize().GetAsFraction();
}
void AgentHandlers::Command_ROTN( CAOSMachine& vm )
{
	...
	vm.GetTarg().GetAgentReference().GetPresence().
		SetSpriteFormat(noOfSpritesForEachRotation, noOfRotations);
}
...
void AgentHandlers::Command_AVEL( CAOSMachine& vm )
{
	...
	vm.GetTarg().GetAgentReference().GetPresence().SetAngularVelocity(Angle(amount));
}
...
{
	vm.ValidateTarg();
	return vm.GetTarg().GetAgentReference().GetPresence().GetFacingAngle().GetAsFraction();
}
...
{
	vm.ValidateTarg();
	Vector2D agentCentre = vm.GetTarg().GetAgentReference().GetPresence().GetPosition();
...
void AgentHandlers::SubCommand_NEW_SIMP( CAOSMachine& vm )
{
	...
	agent.GetAgentReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
}
...
void AgentHandlers::SubCommand_NEW_VHCL( CAOSMachine& vm )
{
	...
	v.GetVehicleReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
}
...
void AgentHandlers::SubCommand_NEW_COMP( CAOSMachine& vm )
{
	...
	agent.GetCompoundAgentReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
}
...
void AgentHandlers::SubCommand_NEW_CREA( CAOSMachine& vm )
{
	...
	agent = theAgentManager.CreateSkeletalCreature(family, gene, gene_index, sex, variant);

	vm.SetTarg( agent );

	while (!vm.GetTarg().GetSkeletalCreatureReference().AreYourBodyPartsFullyFormed())
	{
	...
	agent.GetSkeletalCreatureReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
}
...
void AgentHandlers::SubCommand_NEW_CRAG( CAOSMachine& vm )
{
	...
	agent.GetCreatureReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
}
...
void AgentHandlers::Command_GALL( CAOSMachine& vm )
{
	...
	if(found)
	{
		...
		pathAgain.LowerCase();
		vm.GetTarg().GetAgentReference().SetGallery(pathAgain, baseindex, vm.GetPart());
	}
}
...
void AgentHandlers::Command_NEWC( CAOSMachine& vm )
{
	...
	if(!vm.IsBlocking())
	{
		...
		AgentHandle agent = theAgentManager.CreateSkeletalCreature(family, gene, gene_index, sex, variant);
	...
	else
	{
		...
		if(vm.GetTarg().GetSkeletalCreatureReference().AreYourBodyPartsFullyFormed())
		{
			theAgentManager.AddCreatureToUpdateList(vm.GetTarg());
			vm.GetTarg().GetSkeletalCreatureReference().ExecuteScriptForEvent(SCRIPTCONSTRUCTOR, INTEGERZERO, INTEGERZERO, INTEGERZERO);
			vm.UnBlock();
		}
		else
		{
...
#ifdef C2D_DIRECT_DISPLAY_LIB
	vm.GetTarg().GetAgentReference().CreateAnimStrip(blockWidth,blockHeight,vm.GetPart());
#else
...
{	
	...
	else
	{
		...
		if(a != NULLHANDLE)
			a.GetAgentReference().TryToBeDropped(vm.GetTarg(), INTEGERZERO, INTEGERZERO, true);
	}
