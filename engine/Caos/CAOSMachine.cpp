...
bool CAOSMachine::Write(CreaturesArchive &ar) const {
  ...
  //	int				myQuanta;		// number of steps left for this
  //Update() 	OpType			myCurrentCmd;	// currently executing
  //cmd 	OpType	mySecondaryOp;
...
void CAOSMachine::FormatErrorPos(std::ostream &out, int pos,
  ...
  out << startdot << source.substr(start, pos - start) << "{@}"
