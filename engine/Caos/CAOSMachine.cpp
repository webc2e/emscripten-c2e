...
#include "DebugInfo.h" // error formatting to print source
#include "../App.h" // theMainWindow, Catalogue
#include "../Camera/MainCamera.h" // messagebox prepare for
#include "../Display/Window.h" // single step stuff
#include "../Agents/AgentHandle.h"

#include "CAOSConstants.h"
#include "CAOSMachine.h"
class Camera; // current camera
...
void CAOSMachine::ThrowRunError( const std::exception& e )
{
	throw CAOSMachineRunError(e.what());
}
...
void CAOSMachine::ThrowRunError( int stringid, ... )
{
	...
	throw CAOSMachineRunError( buf );
}

void CAOSMachine::ThrowTagRunError( const char* tag, int stringid, ... )
{
	char buf[ 4096 ];

	// note - this _could_ throw a Catalogue::Err!
	std::string fmt = theCatalogue.Get( tag, stringid );
	va_list args;
	va_start( args, stringid );
	vsprintf( buf, fmt.c_str(), args );
	va_end( args );
...
SkeletalCreature& CAOSMachine::GetSkeletalCreatureTarg()
{
	ValidateTarg();
	if (!myTarg.IsSkeletalCreature())
		ThrowInvalidAgentHandle( sidInvalidCreatureTARG );

	return myTarg.GetSkeletalCreatureReference();
}
...
#else
				// TODO: dialog box here - but not urgeny really.  I think
				// everyone always just chooses abort.
				bool abort = true;
...
bool CAOSMachine::Write(CreaturesArchive &ar) const
{
	...
	for( i=0; i<myAgentStack.size(); ++i )
		ar << myAgentStack[i];
	ar << (int)myStringStack.size();
	for( i=0; i<myStringStack.size(); ++i )
		ar << myStringStack[i];
...
bool CAOSMachine::Read(CreaturesArchive &ar)
{
	...
	if(version >= 3)
	{
		...
		if (version >= 38)
		{
			...
			{
				ar >> tmp_str;
				myStringStack.push_back(tmp_str);
			}
		}
