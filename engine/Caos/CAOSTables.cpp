...
OpSpec ourSubCommandTable_NEW[] = {
    ...
    OpSpec("CREA", "iaiii", "family gene_agent gene_slot sex variant",
           categoryCreatures,
           ...
           "and 8.  See also @#NEWC@."),
    OpSpec("CRAG", "iaiiisiii",
           "family gene_agent gene_slot sex variant sprite_file image_count "
           "first_image plane",
           categoryCreatures,
           "Makes a non-skeletal creature.  The first five arguments are the "
           "same as @#NEW: CREA@.  The last four, starting from the sprite "
...
OpSpec ourSubCommandTable_HIST[] = {
    ...
    OpSpec("UTXT", "sis", "moniker event_no new_value", categoryHistory,
           "For the given life event, sets the user text."),
    OpSpec("NAME", "ss", "moniker new_name", categoryHistory,
           "Renames the creature with the given moniker."),
    OpSpec("WIPE", "s", "moniker", categoryHistory,
    ...
    OpSpec(
        "FOTO", "sis", "moniker event_no new_value", categoryHistory,
...
OpSpec ourSubIntegerRVTable_HIST[] = {
    OpSpec("COUN", "s", "moniker", categoryHistory,
           ...
           "exist."),
    OpSpec(
        "TYPE", "si", "moniker event_no", categoryHistory,
        ...
        "moniker is whom<br>16 Warped out - exported through a worm hole with "
        "@#NET: EXPO@<br>17 Warped in - imported through a worm hole<p>These "
        "events aren't triggered by the engine, but reserved for CAOS to use "
        "with these numbers:<br>11 Laid by mother<br>12 Laid an egg<br>13 "
        "Photographed<p>Other numbers can also be used for custom life events. "
        " Start with numbers 100 and above, as events below that are reserved "
        "for the engine.  You send your own events using @#HIST EVNT@."),
    OpSpec("WTIK", "si", "moniker event_no", categoryHistory,
           "Returns the world tick when the life event happened, for the world "
           "that the event happened in. "),
    OpSpec("TAGE", "si", "moniker event_no", categoryHistory,
    ...
    OpSpec("RTIM", "si", "moniker event_no", categoryHistory,
           ...
           " To display, use @#RTIF@."),
    OpSpec("CAGE", "si", "moniker event_no", categoryHistory,
           "Returns the life stage of the creature when the given life event "
           "happened."),
    OpSpec("GEND", "s", "moniker", categoryHistory,
           ...
           "yet, returns -1."),
    OpSpec("GNUS", "s", "moniker", categoryHistory,
           "Returns the genus of the moniker.  This is 1 for Norn, 2 for "
           "Grendel, 3 for Ettin by convention."),
    OpSpec("VARI", "s", "moniker", categoryHistory,
           "Returns the variant that the creature with the given moniker has "
           "or had.  If the creature hasn't been born yet, returns -1."),
    OpSpec("WVET", "s", "moniker", categoryHistory,
           "Returns 1 if the Creature is a warp hole veteran, 0 otherwise.  "
           "The creature is a veteran if they have ever been exported with "
           "@#NET: EXPO@.  They must have been the main exported creature - "
           "warping as an embryo doesn't make you a veteran."),
    OpSpec("FIND", "sii", "moniker event_type from_index", categoryHistory,
    ...
    OpSpec("FINR", "sii", "moniker event_type from_index", categoryHistory,
...
OpSpec ourSubStringRVTable_HIST[] = {
    ...
    OpSpec("PREV", "s", "moniker", categoryHistory,
    ...
    OpSpec("FOTO", "si", "moniker event_no", categoryHistory,
           "For the given life event, returns the filename of the associated "
           "photograph, or an empty string if there is no photo."),
    OpSpec("NETU", "si", "moniker event_no", categoryHistory,
           "Returns the network identifier of the user when the given life "
           "event happened."),
};

OpSpec ourSubCommandTable_FILE[] = {
    OpSpec(
        "OOPE", "isi", "directory filename append", categoryFiles,
        ...
        "exits.<p>Directory is 0 for the current world's journal directory, 1 "
        "for the main journal directory, or 2 for the @#GAME@ "
        "@#engine_other_world@ world's journal directory.  Set append to 1 to "
        "add to the end of the file, or 0 to replace any existing file."),
    OpSpec("OCLO", "", "", categoryFiles,
           "Disconnects anything which is attached to the output stream.  If "
           "this is a file, then the file is closed."),
    OpSpec("OFLU", "", "", categoryFiles,
           "Flush output stream.  If it is attached to a disk file, this will "
           "force any data in the buffer to be written to disk."),
    OpSpec(
        "IOPE", "is", "directory filename", categoryFiles,
        ...
        "directory, 1 for the main journal directory, or 2 for the @#GAME@ "
        "@#engine_other_world@ world's journal directory."),
    OpSpec("ICLO", "", "", categoryFiles,
           "Disconnects anything which is attached to the input stream.  If "
           "this is a file, then the file is closed."),
    OpSpec(
        "JDEL", "is", "directory filename", categoryFiles,
    ...
    OpSpec("GLOB", "is", "directory filespec", categoryFiles,
           "This globs a journal directory (0 for world one, 1 for main one) "
           "for the filespec provided. As all this can do is list files, it "
           "does not worry about where you look relative to the journal "
           "directory in question. Use this with care. <p>Having globbed a "
           "directory, the listing is available on the input stream as a "
           "number, followed by the names of each file. To read - ise @#INOK@, "
           "@#INNI@ and @#INNL@. Once you have finished, remember to do a "
           "@#FILE ICLO@ to remove the glob output from the VM."),
};

OpSpec ourSubCommandTable_GENE[] = {
    OpSpec("CROS", "aiaiaiiiii",
    ...
    OpSpec("MOVE", "aiai", "dest_agent dest_slot source_agent source_slot",
           categoryGenetics, "Moves a genome from one slot to another."),
    OpSpec("KILL", "ai", "agent slot", categoryGenetics,
           "Clears a genome slot."),
    OpSpec(
        "LOAD", "ais", "agent slot gene_file", categoryGenetics,
    ...
    OpSpec("CLON", "aiai", "dest_agent dest_slot source_agent source_slot",
           categoryGenetics,
...
OpSpec ourSubCommandTable_PRAY[] = {
    OpSpec(
        "REFR", "", "", categoryResources,
        "This command refreshes the engine's view of the Resource directory. "
        "Execute this if you have reason to believe that the files in the "
        "directory may have changed. It only detects changes if there is a new "
        "file or a deleted file - if a file has only changed it won't notice.  "
        "This is awkward during development, you can use @#PRAY KILL@ to kill "
        "the old file before copying the new one over.  PRAY REFR forces a "
        "@#PRAY GARB@ to happen automatically."),
    OpSpec(
        "GARB", "i", "force", categoryResources,
...
OpSpec ourSubIntegerRVTable_PRAY[] = {
    OpSpec("COUN", "s", "resource_type", categoryResources,
    ...
    OpSpec("AGTI", "ssi", "resource_name integer_tag default_value",
           categoryResources,
           ...
           "@#PRAY AGTS@."),
    OpSpec(
        "DEPS", "si", "resource_name do_install", categoryResources,
    ...
    OpSpec(
        "FILE", "sii", "resource_name resource_type do_install",
        categoryResources,
    ...
    OpSpec("TEST", "s", "resource_name", categoryResources,
    ...
    OpSpec(
        "INJT", "siv", "resource_name do_install report_var", categoryResources,
    ...
    OpSpec(
        "EXPO", "s", "chunk_type", categoryResources,
        ...
        "some form.<p>The chunk type should be used to find the creature again "
        "to import it.  In Creatures 3, most exported creatures have a chunk "
        "type EXPC, and the starter family uses SFAM.<p>For new games, you "
        "should not use SFAM, as its data would get confused with that of an "
        "EXPC with the same moniker. This is for backwards compatibility with "
        "Creatures 3's use of SFAM, which works because the CAOS code "
        "guarantees different monikers.<p>For other chunk names, creatures "
        "exported with a different type are kept entirely separately, and will "
        "not get confused with each other. The chunk type is added to the end "
        "of the moniker to form the chunk name.<p>"
        "The exported creature has some fields associated with it, that can be "
        "read by @#PRAY AGTI@ or @#PRAY AGTS@ before importing:<br>\"Exported "
        "At World Time\" <i>integer</i>	<br>\"Creature Age In Ticks\" "
        "<i>integer</i>	<br>\"Exported At Real Time\" <i>integer</i>	"
        "<br>\"Creature Life Stage\" <i>integer</i>	<br>\"Exported From "
        "World Name\" <i>string</i>	<br>\"Exported From World UID\" "
        "<i>string</i>	<br>\"Native Network User\" <i>string</i>	"
        "<br>\"Last Network User\" (could be faked, @#NET: FROM@ is safer) "
        "<i>string</i>	<br>\"Creature Name\" <i>string</i>	<br>\"Gender\" "
        "<i>integer</i>	<br>\"Genus\" <i>integer</i>	<br>\"Variant\" "
        "<i>integer</i>	<br>\"Head Gallery\" <i>string</i> (this is calculated "
        "on the sending computer, so the file may be missing on the receiving "
        "one - try @#LIMB@ instead)<br>\"Pregnancy Status\" "
        "<i>integer</i><p>In addition you can add custom fields by setting "
        "@#NAME@ variables on the Creature before export.  Any strings or "
        "integers whose name begin \"Pray Extra \" are added as entries to the "
        "export file."),
    OpSpec(
        "IMPO", "sii", "moniker_chunk actually_do_it keep_file",
        categoryResources,
        "This function imports the creature with the requested moniker and "
        "chunk type. Returns one of the following codes:<br>0 - success<br>1 - "
        "couldn't reconcile histories so creature was cloned<br>2 - moniker "
        "not found in PRAY system<br>3 - unused error code<br>4 - internal / "
        "file format error<p>Set actually_do_it to 1 to try and perform the "
        "import, or 0 to perform a query giving just the return value.  You "
        "can use the query to test if the creature is available, and if the "
        "creature would have to be cloned upon importing, and warn the user.  "
        "The new creature is @#TARG@etted after import.  If you set keep file "
        "to 1, then the exported file won't be deleted (moved to the porch).  "
        "The creature will appear in the same place that it was exported, but "
        "as with @#NEW: CREA@, it will be in limbo, and won't function until "
    ...
    OpSpec("KILL", "s", "resource_name", categoryResources,
           "Deletes the resource file which contains the specified chunk. This "
           "is permanent and irreversible. Returns 1 if there was such a chunk "
           "and file, or 0 if there wasn't."),
};

OpSpec ourSubStringRVTable_PRAY[] = {
    OpSpec("PREV", "ss", "resource_type last_known", categoryResources,
           "This returns the name of the resource chunk directly before the "
           "named one, given that they are of the same type. It loops when it "
           "reaches the end.  If the named resource cannot be found in the "
           "list of resources of the type specified, then the first resource "
           "of that type is returned. This call pairs with @#PRAY NEXT@.  "
           "Compare @#PRAY BACK@."),
    OpSpec("NEXT", "ss", "resource_type last_known", categoryResources,
           "This returns the name of the resource chunk directly after the "
           "named one, given that they are of the same type. It loops when it "
           "reaches the end.  If the named resource cannot be found in the "
           "list of resources of the type specified, then the last resource of "
           "that type is returned. This call pairs with @#PRAY PREV@.  Compare "
           "@#PRAY FORE@."),
    OpSpec("AGTS", "sss", "resource_name string_tag default_value",
           categoryResources,
           ...
           "AGTI@."),
    OpSpec("BACK", "ss", "resource_type last_known", categoryResources,
           "Like @#PRAY PREV@, only doesn't loop at the end.  If you go beyond "
           "the first entry then it returns an empty string."),
    OpSpec("FORE", "ss", "resource_type last_known", categoryResources,
           "Like @#PRAY NEXT@, only doesn't loop at the end.  If you go beyond "
           "the last entry then it returns an empty string."),
};

OpSpec ourSubCommandTable_MESG[] = {
    OpSpec(
        "WRIT", "ai", "agent message_id", categoryAgents,
    ...
    OpSpec("WRT+", "aimmi", "agent message_id param_1 param_2 delay",
           categoryAgents,
...
OpSpec ourSubCommandTable_STIM[] = {
    OpSpec(
        "SHOU", "if", "stimulus strength", categoryCreatures,
    ...
    OpSpec("SIGN", "if", "stimulus strength", categoryCreatures,
           "Send a stimulus to all creatures who can see @#OWNR@."),
    OpSpec("TACT", "if", "stimulus strength", categoryCreatures,
           "Send a stimulus to all creatures who are touching @#OWNR@."),
    OpSpec("WRIT", "aif", "creature stimulus strength", categoryCreatures,
...
OpSpec ourSubCommandTable_URGE[] = {
    OpSpec("SHOU", "fif", "noun_stim verb_id verb_stim", categoryCreatures,
    ...
    OpSpec("SIGN", "fif", "noun_stim verb_id verb_stim", categoryCreatures,
           "Urge all creatures who can see @#OWNR@ to perform an action on "
           "@#OWNR@."),
    OpSpec("TACT", "fif", "noun_stim verb_id verb_stim", categoryCreatures,
           "Urge all creatures who are touching @#OWNR@ to perform an action "
           "on @#OWNR@."),
    OpSpec("WRIT", "aifif", "creature noun_id noun_stim verb_id verb_stim",
           categoryCreatures,
...
OpSpec ourSubCommandTable_ORDR[] = {
    OpSpec("SHOU", "s", "speech", categoryCreatures,
           "Sends a spoken command from target to all creatures that can hear "
           "it.  As well as the usual speech that Creatures can understand, "
           "there is a special syntax that can be used for learning machines.  "
           "This is as follows:<p><pre>learn|perfect "
           "verb|noun|drive|qualifier|special|personal|nice_drive [number] "
           "[word]</pre> <p>Here the | means \"or\", the [number] is the id "
           "slot to learn, and [word] is the new text to fill that word in "
           "with.  \"perfect\" makes the Creature learn it perfectly straight "
           "away, whereas \"learn\" does a gradually learning.  See the "
           "Creatures 3 learning machine cos file for an example use."),
    OpSpec(
        "SIGN", "s", "speech", categoryCreatures,
        "Sends a spoken command from target to all creatures that can see it."),
    OpSpec("TACT", "s", "speech", categoryCreatures,
           "Sends a spoken command from target to all creatures that are "
           "touching it."),
    OpSpec("WRIT", "as", "creature speech", categoryCreatures,
           "Sends a spoken command from target to the specified creature."),
};

OpSpec ourSubCommandTable_GIDS[] = {
    OpSpec("ROOT", "", "", categoryScripts,
           "Output the family numbers for which there are scripts in the "
           "scriptorium.  List is space delimited."),
    OpSpec("FMLY", "i", "family", categoryScripts,
           "Output the genus numbers for which there are scripts in the "
           "scriptorium for the given family.  List is space delimited."),
    OpSpec("GNUS", "ii", "family genus", categoryScripts,
           "Output the species numbers for which there are scripts in the "
           "scriptorium for the given family and genus.  List is space "
           "delimited."),
    OpSpec("SPCS", "iii", "family genus species", categoryScripts,
           "Output the event numbers of scripts in the scriptorium for the "
           "given classifier.  List is space delimited."),
};

// PRT: subcommands:
OpSpec ourSubCommandTable_PRT[] = {
    OpSpec(
        "INEW", "issiii", "id name description x y message_num", categoryPorts,
    ...
    OpSpec("IZAP", "i", "id", categoryPorts,
           "Remove the specified input port."),
    OpSpec(
        "ONEW", "issii", "id name description x y", categoryPorts,
    ...
    OpSpec("OZAP", "i", "id", categoryPorts,
           "Remove the specified output port."),
    OpSpec("JOIN", "aiai", "source_agent output_id dest_agent input_id",
           categoryPorts,
    ...
    OpSpec("KRAK", "aii", "agent in_or_out port_index", categoryPorts,
...
OpSpec ourSubStringRVTable_PRT[] = {
    OpSpec("NAME", "aii", "agent in_or_out port_index", categoryPorts,
...
OpSpec ourSubAgentRVTable_PRT[] = {
    OpSpec("FRMA", "i", "inputport", categoryPorts,
...
OpSpec ourSubCommandTable_PAT[] = {
...
#ifdef C2D_DIRECT_DISPLAY_LIB
    OpSpec("TEXT", "isiddiisiiiiii",
           "part_id sprite_file first_image rel_x rel_y rel_plane message_id "
           "font_sprite font_width fontHeight red blue green bold",
           categoryCompound,
    ...
    OpSpec("FIXD", "isiddisiiiiii",
           "part_id sprite_file first_image rel_x rel_y rel_plane font_sprite "
           "font_width fontHeight red blue green bold",
           categoryCompound,
    ...
    OpSpec(
        ...
        "font_sprite",
        categoryCompound,
        "Creates a text entry part.  Gains the focus when you click on it, or "
        "with the @#FCUS@ command.  Sends the message_id when return is "
        "pressed - a good place to use @#PTXT@ to get the text out, and to set "
        "the focus elsewhere.  Set message_id to 0 to not call any script, or "
        "to -1 to not send any message and instead insert a carriage return."),
    OpSpec("FIXD", "isiddis",
           "part_id sprite_file first_image rel_x rel_y rel_plane font_sprite",
           categoryCompound,
           "Create a fixed text part. The text is wrapped on top of the "
    ...
    OpSpec("KILL", "i", "part_id", categoryCompound,
           "Destroys the specified part of a compound agent.  You can't "
           "destroy part 0."),
    OpSpec("MOVE", "idd", "part_id rel_x rely", categoryCompound,
           "Moves a compound part to the new relative position specified."),
};

OpSpec ourSubCommandTable_BRN[] = {
    OpSpec("SETN", "iiif", "lobe_number neuron_number state_number new_value",
           categoryBrain, "Sets a neuron weight."),
    OpSpec("SETD", "iiif",
           "tract_number dendrite_number weight_number new_value",
           categoryBrain, "Sets a dendrite weight."),
    OpSpec("SETL", "iif", "lobe_number line_number new_value", categoryBrain,
           "Sets a lobe's SV rule float value."),
    OpSpec("SETT", "iif", "tract_number line_number new_value", categoryBrain,
           "Sets a tract's SV rule float value."),
    OpSpec("DMPB", "", "", categoryBrain,
           "Dumps the sizes of the binary data dumps for current lobes and "
           "tracts."),
    OpSpec("DMPL", "i", "lobe_number", categoryBrain,
           "Dumps a lobe as binary data."),
    OpSpec("DMPT", "i", "tract_number", categoryBrain,
           "Dumps a tract as binary data."),
    OpSpec("DMPN", "ii", "lobe_number neuron_number", categoryBrain,
           "Dumps a neuron as binary data."),
    OpSpec("DMPD", "ii", "tract_number dendrite_number", categoryBrain,
           "Dumps a dendrite as binary data."),
};

OpSpec ourSubCommandTable_DBG[] = {
    OpSpec("PAWS", "", "", categoryDebug,
    ...
    OpSpec("TOCK", "", "", categoryDebug,
           ...
           "game clock."),
    OpSpec("FLSH", "", "", categoryDebug,
           "This flushes the system's input buffers - usually only useful if "
           "@#DBG: PAWS@ed."),
    OpSpec("POLL", "", "", categoryDebug,
           "This takes all of the @#DBG: OUTV@ and @#DBG: OUTS@ output to date "
           "and writes it to the output stream."),
    OpSpec("OUTV", "d", "value", categoryDebug,
           "Send a number to the debug log - use @#DBG: POLL@ to retrieve."),
    OpSpec("OUTS", "s", "value", categoryDebug,
           "Send a string to the debug log - use @#DBG: POLL@ to retrieve."),
    OpSpec(
        "PROF", "", "", categoryDebug,
        "Sends agent profile information to the output stream.  This gives you "
        "data about the time the engine spends running the update and message "
        "handling code for each classifier.  The data is measured from engine "
        "startup, or the point marked with @#DBG: CPRO@.  It's output in comma "
        "separated value (CSV) format, so you can load it into a spreadsheet "
        "(e.g. Gnumeric or Excel) for sorting and summing."),
    OpSpec("CPRO", "", "", categoryDebug,
           "Clears agent profiling information.  Measurements output with "
           "@#DBG: PROF@ start here."),
    OpSpec("HTML", "i", "sort_order", categoryDebug,
           "Sends CAOS documentation to the output stream.  Sort order is 0 "
           "for alphabetical, 1 for categorical."),
    OpSpec("ASRT", "c", "condition", categoryDebug,
           "Confirms that a condition is true.  If it isn't, it displays a "
           "runtime error dialog."),
    OpSpec("WTIK", "i", "new_world_tick", categoryDebug,
    ...
    OpSpec("TACK", "a", "follow", categoryDebug,
...
OpSpec ourSubIntegerRVTable_CD[] = {
    OpSpec("FRQL", "", "", categoryCDPlayer,
           "Returns the average value for the lowest frequencies detected in "
           "the CD player for the current tick."),
    OpSpec("FRQM", "", "", categoryCDPlayer,
           "Returns the average value for the medium frequencies detected in "
           "the CD player for the current tick."),
    OpSpec("FRQH", "", "", categoryCDPlayer,
           "Returns the average value for the highest frequencies detected in "
           "the CD player for the current tick."),
};

OpSpec ourSubCommandTable_CD[] = {
    OpSpec("INIT", "", "", categoryCDPlayer,
           "Let the game know that you wish to use the cd player.  This will "
           "shut down all in game sounds and music as the mixer is needed to "
           "gauge the frequency spectrum."),
    OpSpec("PLAY", "ii", "first_track last_track", categoryCDPlayer,
           "Tell the CD Player to play the given track."),
    OpSpec("STOP", "", "", categoryCDPlayer, "Stop the cd player."),
    OpSpec("PAWS", "i", "on_off", categoryCDPlayer,
           "Pause the CD player if the parameter is greater than zero, to "
           "continue playing a previous paused track set the parameter to 1."),
    OpSpec("EJCT", "", "", categoryCDPlayer, "Open the CD tray."),
    OpSpec("SHUT", "", "", categoryCDPlayer,
           "Tell the game that you have finished with the cd player.  This "
           "will reinstate the in game sounds and music."),
};
...
OpSpec ourCommandTable[] = {
    ...
    OpSpec("ABSV", GeneralHandlers::Command_ABSV, "v", "var", categoryVariables,
           "Makes a variable positive (its absolute value), so if var is "
           "negative var = 0 - var, otherwise var is left alone."),
    ...
    OpSpec("PART", AgentHandlers::Command_PART, "i", "part_id",
           ...
           "@#ANIM@, amongst others, act on that part of a compound agent.  To "
           "find what parts there are on an agent use @#PNXT@."),
    ...
    OpSpec("DELN", AgentHandlers::Command_DELN, "m", "variable_name",
           categoryVariables,
           "Deletes the specified @#NAME@ variable on @#TARG@."),
    OpSpec("DELE", GeneralHandlers::Command_DELE, "s", "variable_name",
           categoryVariables, "Deletes the specified @#EAME@ variable."),
    ...
    OpSpec("MMSC", SoundHandlers::Command_MMSC, "iis", "x y track_name",
           ...
           "coordinates."),
    OpSpec(
        "PTXT", CompoundHandlers::Command_PTXT, "s", "text", categoryCompound,
        "Set string of current text part.  Use @#PAT: TEXT@ or @#PAT: FIXD@ to "
        "make a text part, and @#PART@ to set the current part.<p>In the "
        "original display engine, you can use special tags to set @#TINT@ "
        "colours for characters. Use something like &lt;tint 255 255 0&gt; to "
        "begin colouring, and &lt;tint&gt; to end.  The tint tag takes up to "
        "five parameters as the @#TINT@ command, they all default to 128 if "
        "not specified.  Less than and greater than symbols are still printed "
        "if not within a tint tag.  The tags also apply to text entry parts, "
        "but only when the text is initially set with @#PTXT@; they are obeyed "
        "in a character count fashion during editing, but not updated."),
    OpSpec("NORN", CreatureHandlers::Command_NORN, "a", "creature",
           categoryCreatures,
           "Chooses the active creature.  Script 120 (@#Selected Creature "
           "Changed@) is then executed on all agents which have it."),
    ...
    OpSpec("VOLM", SoundHandlers::Command_VOLM, "ii", "channel volume",
           categorySounds,
           "Set overall the volume of the sound effects (channel 0), the MIDI "
           "(channel 1) or the generated music (channel 2).  Values range from "
           "-10000 (silent) to 0 (loudest)."),
    ...
    OpSpec("LOAD", GeneralHandlers::Command_LOAD, "s", "world_name",
           ...
           "@#QUIT@ for important information about using @#INST@.  See "
           "@#BOOT@ and @#engine_no_auxiliary_bootstrap_nnn@ for extra "
           "information about bootstrapping a world."),
    ...
    OpSpec("PSWD", GeneralHandlers::Command_PSWD, "s", "password",
           categoryWorld,
    ...
    OpSpec(
        "FRMT", CompoundHandlers::Command_FRMT, "iiiiiii",
        ...
        "extra pixels to insert between characters. Values for justification "
        "are 0 - Left, 1 - Right, 2 - Center, 4 - Bottom, 8 - Middle, 16 - "
        "Last Page Scroll (if you add extra text to the part and show the last "
        "page, it will scroll upwards).  You may add mutually compatible "
        "numbers.  The default format values are 8 8 8 8 0 0 0."),
    OpSpec("SNAP", DisplayHandlers::Command_SNAP, "siiiii",
           ...
           "zoom parameter should be between 0 and 100. 100 means at original "
           "size, 50 means half size etc.  It makes a new image file in the "
           "world images directory - you can use it to make agents and parts "
           "as with any image file.  Call @#SNAX@ first to check your filename "
           "isn't already in use in any images directory.  When you have "
    ...
    OpSpec("TINT", DisplayHandlers::Command_TINT, "iiiii",
           ...
           "properties of the gallery. See @#TINO@ for a quicker version that "
           "tints only one frame."),
    ...
    OpSpec("MEMX", DebugHandlers::Command_MEMX, "", "", categoryDebug,
           "Windows only.  Sends information about the memory allocated to the "
           "output stream.  In order, these are the Memory Load (unknown), "
           "Total Physical (size in bytes of physical memory), Available "
           "Physical (free physical space), Total Page File (maximum possible "
           "size of page file), Available Page File (size in bytes of space "
           "available in paging file), Total Virtual (size of user mode "
           "portion of the virtual address space of the engine), Available "
           "Virtual (size of unreserved and uncommitted memory in the user "
           "mode portion of the virtual address space of the engine)."),
    OpSpec("CALL", AgentHandlers::Command_CALL, "imm",
           "event_no param_1 param_2", categoryAgents,
           "Calls a subroutine script on the owner with the specified event "
           "number.  When that script finishes the current script is resumed.  "
           "No variables are shared between the two scripts so any return "
           "values must go through OVs.  The called script starts in the same "
           "INST state as the calling script, however, it may use SLOW or INST "
           "to override this initial state.  In addition, when the script "
           "returns to the calling script, the INST state is reset to what it "
           "was before the CALL command, so CALL preserves INSTness in the "
           "calling script.  So if the caller script is in an INST then the "
           "called script will inherit that, any change in the called script "
           "to cancel this (such as OVER, WAIT, SLOW etc) will only affect the "
           "called script... when execution returns to the caller script it "
           "will still be in whatever state it was in before."),
#ifdef C2D_DIRECT_DISPLAY_LIB
    OpSpec("BLCK", AgentHandlers::Command_BLCK, "ii",
           "block_width block_height", categoryAgents,
           "Set the block width and height for the current agent and create "
           "the Animation strip to display the agent.  This command is only "
           "for the direct display engine."),
    OpSpec("SCLE", DisplayHandlers::Command_SCLE, "fi", "scaleBy yesOrNo",
           categoryAgents,
           "Scales an image from the @#TARG@ agent's sprite file by the given "
           "percentage. The scale factor always refers to the original size of "
           "the image.  The second parameter turns scaling on or off.  For the "
           "direct display engine scaling is done on the fly.  Otherwise the "
           "whole sprite file will be cloned and scaled, note that this takes "
           "up lots of memory."),
#endif
    OpSpec(
        "ALPH", DisplayHandlers::Command_ALPH, "ii", "alpha_value yesOrNo",
        categoryAgents,
        "The agent will be drawn alpha blended against the background by the "
        "given value - from 256 for invisible to 0 for completely solid.  For "
        "compound agents set the @#PART@ to affect a particular part or to -1 "
        "to affect all parts.  The second parameter switches alpha blending on "
        "(1) or off (0).  Alpha graphics are drawn much slower, so use "
        "sparingly and turn it off completely rather than using an intensity "
        "value of 0 or 256.  At the moment alpha channels only work on "
        "compressed, non-mirrored, non-zoomed sprites."),
#ifdef C2D_DIRECT_DISPLAY_LIB
    OpSpec(
        "SHAD", DisplayHandlers::Command_SHAD, "iiii",
        "shadow_value x_offset y_offset yesOrNo", categoryAgents,
        "A shadow will be drawn offset from the agent by the given x y "
        "co-ordinates.  The intensity of the shadow depends on the "
        "shadow_intensity value.  This only works when using the direct "
        "display engine.  The second parameter switches the shadow on or off."),
    OpSpec("STRC", DisplayHandlers::Command_STRC, "iii",
           "stretched_width stretched_height yesOrNo", categoryAgents,
           "The agent will be drawn stretched widthways, given the new "
           "stretched width.  This only works when using the direct display "
           "engine.  The second parameter switches stretching on or off."),
    OpSpec("FLIP", AgentHandlers::Command_FLIP, "i", "onOroff", categoryAgents,
           "Tell the agent to draw the current sprite vertically flipped (send "
           "1 as a parameter) or normally (send 0 as a parameter). This only "
           "works when using the direct display engine."),
    OpSpec("ROTA", AgentHandlers::Command_ROTA, "i", "angle", categoryAgents,
           "DirectDraw version does not work will try to find another way.  "
           "Tell the agent to draw the current sprite rotated clockwise by the "
           "given angle a value between 0 and 360. This only works when using "
           "the direct display engine.  "),
    OpSpec("OUTL", AgentHandlers::Command_OUTL, "iiii",
           "onOroff red green blue", categoryAgents,
           "Detect and outline the edges of an agent with a specified colour."),
#endif
    OpSpec("PLMD", CreatureHandlers::Command_PLMD, "is",
           "tract_index, filename", categoryCreatures,
           "Dumps out all the dendrite learned information of the specified "
           "tract to a file (will be changed to sync with Palm)."),
    OpSpec("PLMU", CreatureHandlers::Command_PLMU, "is",
           "tract_index, filename", categoryCreatures,
           "Configures the dendrites in the specified tract with the data in "
           "the file (will be changed to sync with Palm)."),
    OpSpec("NOTV", GeneralHandlers::Command_NOTV, "v", "var", categoryVariables,
           "Peform a bitwise NOT on an integer variable."),
    OpSpec("BANG", GeneralHandlers::Command_BANG, "", "", categoryDebug,
           "Causes a division by zero exception in the processor, to test the "
           "engine's error handling routines."),
    OpSpec(
        "BUZZ", GeneralHandlers::Command_BUZZ, "i", "interval", categoryTime,
        "Sets the ideal interval in milliseconds between each tick.  However "
        "fast the machine, it won't tick quicker than this, but it might tick "
        "slower.  You can find the actual time taken with @#RACE@.  Changing "
        "this from the default value of 50 midgame will damage profiling and "
        "seasons.  Things such as creature brains are designed for the default "
        "50 millisecond update interval.  Change this with caution!"),
    OpSpec("CALG", CreatureHandlers::Command_CALG, "ii",
           "category_id category_representative_algorithm_id",
           categoryCreatures,
           "Choose how a creature (TARG) decides which particular object in a "
           "category to look at.  PICK_NEAREST_IN_X_DIRECTION=0, "
           "PICK_A_RANDOM_ONE=1, PICK_NEAREST_IN_CURRENT_ROOM=2, "
           "PICK_NEAREST_TO_GROUND=3, PICK_RANDOM_NEAREST_IN_X_DIRECTION=4."),
    OpSpec("NAMN", AgentHandlers::Command_NAMN, "v", "previous",
           categoryVariables,
           "Enumerates through @#NAME@ variable names, roughly as @#GAMN@ does "
           "through @#GAME@ variables.  Start and end with an empty string."),
    OpSpec("FVEL", AgentHandlers::Command_FVEL, "f", "amount_in_pixels",
           categoryMotion, "Set forwards velocity."),
    OpSpec("SVEL", AgentHandlers::Command_SVEL, "f", "amount_in_pixels",
           categoryMotion, "Set sideways velocity."),
    OpSpec("AVEL", AgentHandlers::Command_AVEL, "f",
           "amount_in_fraction_of_whole_circle", categoryMotion,
           "Set angular velocity."),
    OpSpec("SPIN", AgentHandlers::Command_SPIN, "f",
           "amount_in_fraction_of_whole_circle", categoryMotion,
           "Rotate to a particular facing."),
    OpSpec("ADMP", AgentHandlers::Command_ADMP, "f", "damping_factor",
           categoryMotion,
           "Damp angular velocity by this much each tick.  The value is from "
           "0.0 to 1.0 where 0.0 means no damping, 1.0 maximum."),
    OpSpec("FDMP", AgentHandlers::Command_FDMP, "f", "damping_factor",
           categoryMotion,
           "Damp forward velocity by this much each tick.  The value is from "
           "0.0 to 1.0 where 0.0 means no damping, 1.0 maximum."),
    OpSpec("SDMP", AgentHandlers::Command_SDMP, "f", "damping_factor",
           categoryMotion,
           "Damp sideways velocity by this much each tick.  The value is from "
           "0.0 to 1.0 where 0.0 means no damping, 1.0 maximum."),
    OpSpec("ROTN", AgentHandlers::Command_ROTN, "ff",
           "no_of_sprites_for_each_rotation no_of_rotations", categoryMotion,
           "For automatic change of sprite when the agent rotates the engine "
           "assumes that the sprite file is stored with all the sprites for "
           "one rotation together starting with pointing north."),
    OpSpec("VARC", AgentHandlers::Command_VARC, "f", "view_arc_size",
           categoryMotion, "[not implemented yet]"),
    OpSpec("DROP", AgentHandlers::Command_DROP, "", "", categoryAgents,
           "Force the @#TARG@ to drop what it is carrying.  this will try to "
           "find a safe place for the agent to fall."),
#ifndef C2D_DIRECT_DISPLAY_LIB
    OpSpec("TINO", DisplayHandlers::Command_TINO, "iiiii",
           "red_tint green_tint blue_tint rotation swap", categoryAgents,
           "Like @#TINT@ but only tints the current frame.  The other frames "
           "are no longer available in the gallery, it becomes a one frame "
           "sprite file.  Original display engine only."),
#endif
    OpSpec("DOIN", CreatureHandlers::Command_DOIN, "i",
           "no_of_instincts_to_process", categoryBrain,
           "Make the creature TARG process N instincts."),
    OpSpec("ADIN", CreatureHandlers::Command_ADIN, "iifi",
           "verb noun qualifier drive", categoryBrain,
           "Add an instinct to the creature's brain queue.  (The instinct is "
           "not processed immediately).  Example:  ADIN 3 4 0.5 7 encourages "
           "(by a factor of 0.5) the creature to do action 3 on category 4 "
           "when drive 7 is high."),
    OpSpec(
        "BOOT", GeneralHandlers::Command_BOOT, "iii",
        "subboot_number folder_number_sum clear_world", categoryCreatures,
        "Loads in a list of numbered bootstrap folders contained within a "
        "folder called subboot. Bootstraps folders numbers are powers of 2, "
        "sum the folder numbers to load those folders.  There is provision for "
        "more than one subboot folder, these should be uniquely numbered. "),
    OpSpec("WEBB", GeneralHandlers::Command_WEBB, "s", "http_url",
           categoryFiles,
           "Launches an external URL in the user's browser. The game engine "
           "adds http:// at the beginning to prevent malicious launching of "
           "programs, so you just need to specify the domain name and path."),
    OpSpec("CATO", CreatureHandlers::Command_CATO, "i", "category",
           categoryAgents,
           "Change the target's category to the one specified.  The default is "
           "-1 which means the category is based on classifier and the "
           "catalogue as described in @#CATI@.  See also @#CATX@ and @#CATA@."),
    OpSpec("CALC", MapHandlers::Command_CALC, "", "", categoryMap,
           "Recalculates all the navigational CAs (warning: slow)."),
    OpSpec("UCLN", AgentHandlers::Command_UCLN, "", "", categoryAgents,
           "Make sure that an agent isn't cloned anymore, this releases the "
           "memory taken up by @#TINT@ing it. Agents are usually cloned for "
           "purposes such as tinting. Don't forget to set the relevant @#PART@ "
           "number for compound agents."),
    OpSpec("MIND", CreatureHandlers::Command_MIND, "i", "state",
           categoryCreatures,
           "Enable (1) or disable (0) the creature's brain (unlike @#ZOMB#@ in "
           "that the brain actually stops processing and it's output is frozen "
           "onto the one noun and verb)."),
    OpSpec("CORE", AgentHandlers::Command_CORE, "ffff",
           "topY bottomY leftX rightX", categoryAgents,
           "Sets the bounding box of the physical core of the object TARG.  "
           "May be set to smaller (or larger) than the sprite's rectangle."),
    OpSpec("DCOR", AgentHandlers::Command_DCOR, "i", "core_on", categoryAgents,
           "Debug command to show the physical core of the TARG agent "
           "graphically."),
    OpSpec("MOTR", CreatureHandlers::Command_MOTR, "i", "state",
           categoryCreatures,
           "Enable (1) or disable (0) the creature's motor faculty, i.e. "
           "whether it sets the IT object and fires off scripts."),
    OpSpec("DSEE", CreatureHandlers::Command_DSEE, "i", "can_see_on",
           categoryAgents,
           "Debug command to show all the agents which can be seen by any "
           "creature."),
    OpSpec("SOUL", CreatureHandlers::Command_SOUL, "ii", "facultyId on",
           categoryCreatures,
           "Enable (1) or disable (0) the update on the creature's faculty as "
           "given by type id as follows: Sensory Faculty (0), Brain (1), Motor "
           "Faculty (2), Linguistic Faculty (3), Biochemistry (4), "
           "Reproductive Faculty (5), Expressive Faculty (6), Music Faculty "
           "(7), Life Faculty (8)"),
    OpSpec("STEP", CreatureHandlers::Command_STEP, "i", "facultyId",
           categoryCreatures,
           "Does one update of the specified faculty (for faculty id see "
           "@#SOUL#@)."),
    OpSpec("JECT", GeneralHandlers::Command_JECT, "si", "cos_file flags",
           categoryScripts,
           "Injects a COS file from the bootstrap directory.  The file is "
           "searched for (case insensitively) in all bootstrap subdirectories. "
           " You must specify the file extension (it doesn't have to be .cos). "
           " Flags is a combination of what you want to inject:<p>1 - Remove "
           "sctipt<br>2 - Event scripts<br>4 - Install script<p>The scripts "
           "(if present) are injected in that order.  So, setting flags to 7 "
           "will fully uninstall and reinstall the cos file.  Error messages "
           "and output are written to the current output stream."),
};
...
OpSpec ourIntegerRVTable[] = {
    ...
    OpSpec("UNID", AgentHandlers::IntegerRV_UNID, "", "", categoryDebug,
           ...
           "programs to persistently refer to an agent for a session.  "
           "Variables can use @#SETA@ to store agent r-values directly for "
           "internal use.  The unique identifier of an agent can change if you "
    ...
    OpSpec(
        "PUPT", AgentHandlers::IntegerRV_PUPT, "ii", "pose x_or_y",
        categoryAgents,
        "Returns the x or y coordinate of the place where target picks agents "
        "up for the given pose.  x_or_y is 1 for x, 2 for y.  The pose is "
        "measured from the absolute base specified in the NEW: command, rather "
        "than the relative base specified by the @#BASE@ command."),
    OpSpec("PUHL", AgentHandlers::IntegerRV_PUHL, "ii", "pose x_or_y",
           categoryAgents,
           "Returns the x or y coordinate of the handle that target is picked "
           "up by for the given pose.  x_or_y is 1 for x, 2 for y.  The pose "
           "is measured from the absolute base specified in the NEW: command, "
           "rather than the relative base specified by the @#BASE@ command."),
    ...
    OpSpec("CATI", CreatureHandlers::IntegerRV_CATI, "iii",
           ...
           " -1 is always returned if none match.  Agents can override their "
           "classifier category with @#CATO@."),
    ...
    OpSpec("RACE", GeneralHandlers::IntegerRV_RACE, "", "", categoryTime,
           ...
           "minimum of 50 milliseconds, or the value set by @#BUZZ@.  It "
           "accounts for all the time in the tick, including event handling "
    ...
    OpSpec(
        ...
        "See also the @#EAME@ variables @#engine_nudge_border_t@ etc."),
    ...
    OpSpec("SINS", GeneralHandlers::IntegerRV_SINS, "sis",
           "main index_to_search_from search", categoryVariables,
           "Looks for the search string as an exact substring of the main "
           "string (<b>s</b>tring <b>in</b> <b>s</b>tring).  Starts searching "
           "at the given index into the main string - to find the first "
           "occurence, set this to 1.  String indices begin at 1.  Returns the "
           "index of the position of the substring, if found, or -1 otherwise. "
           " The search is case sensitive - use @#UPPA@ and @#LOWA@ to convert "
           "all strings first, for a case insensitive search."),
    OpSpec("PART", CompoundHandlers::IntegerRV_PART, "i", "part_id",
           categoryCompound,
           "Returns 1 if the given part number exists on the @#TARG@ agent and "
           "0 if it does not."),
    OpSpec("BUZZ", GeneralHandlers::IntegerRV_BUZZ, "", "", categoryTime,
           "Returns the ideal interval in milliseconds between each tick.  You "
           "can find the actual interval with @#RACE@."),
    OpSpec("CALG", CreatureHandlers::IntegerRV_CALG, "i", "category_id",
           categoryCreatures,
           "Find out which algorithm is currently being used for the creature "
           "TARG to decide which particular object in a category to look at."),
    OpSpec("ABBA", AgentHandlers::IntegerRV_ABBA, "", "", categoryAgents,
           "Returns the absolute base for the current agent/part.  Returns -1 "
           "if an invalid part.  The absolute base is the value passed into "
           "@#NEW: SIMP@ and so on, it is different from the @#BASE@."),
    OpSpec("PNXT", AgentHandlers::IntegerRV_PNXT, "i", "previous_part",
           categoryCompound,
           "Returns the next compound @#PART@ on an agent.  Start by calling "
           "it with -1 to get the first part, and it finishes by returning -1 "
           "when it reaches the end."),
    OpSpec("TINT", AgentHandlers::IntegerRV_TINT, "i", "attribute",
           categoryAgents,
           "Returns a tint value for an agent - currently it works only on "
           "Skeletal Creatures.  Attribute can be:<br>1 - Red<br>2 - "
           "Green<br>3 - Blue<br>4 - Rotation<br>5 - Swap"),
#ifdef C2D_DIRECT_DISPLAY_LIB
    OpSpec("FLIP", AgentHandlers::IntegerRV_FLIP, "", "", categoryAgents,
           "Is the current sprite for this agent vertically flipped (returns "
           "1) or not (returns 0)"),
    OpSpec("ROTA", AgentHandlers::IntegerRV_ROTA, "", "", categoryAgents,
           "By what angle is the current sprite for this agent rotated returns "
           "angle between 0 and 360."),
#endif
    OpSpec("CATA", CreatureHandlers::IntegerRV_CATA, "", "", categoryAgents,
           "Returns the target's category.  This either depends on its "
           "classifier as described in @#CATI@, or is its own individual "
           "override set with @#CATO@."),
    OpSpec("TRAN", AgentHandlers::IntegerRV_TRAN, "ii", "xpos ypos",
           categoryAgents,
           "Test for a transparent pixel, returns 1 if the given x y position "
           "coincides with a transparent pixel on the @#TARG@ agent, otherwise "
           "it will return 0.  This does not work for creatures."),
    OpSpec("VOLM", SoundHandlers::IntegerRV_VOLM, "i", "channel",
           categorySounds,
           "Returns the overall the volume of the sound effects (channel 0), "
           "the MIDI (channel 1) or the generated music (channel 2).  Values "
           "range from -10000 (silent) to 0 (loudest)."),
    OpSpec("HOTP", AgentHandlers::IntegerRV_HOTP, "", "", categoryInput,
           "Returns the number of the compound part under the pointer.  "
           "Returns -1 if no agent is under the pointer, and 0 if the agent is "
           "simple or a skeletal creature.  Transparency of the parts is "
           "ignored, so each part is a rectangle.  Transparency of the agent "
           "as a whole is, however, obeyed.  Planes are also ignored, except "
           "later part numbers are treated as above earlier ones.  See also "
           "@#HOTS@ to find the agent under the pointer."),
    OpSpec(
        "MIND", CreatureHandlers::IntegerRV_MIND, "", "", categoryCreatures,
        "Returns whether the creature TARG's brain is being processed or not."),
    OpSpec("TCOR", AgentHandlers::IntegerRV_TCOR, "ffff",
           "topY bottomY leftX rightX", categoryAgents,
           "Tests setting the bounding box of the physical core of the object "
           "TARG.  May be set to smaller (or larger) than the sprite's "
           "rectangle.  Returns 1 if OK to set (using @#CORE@), 0 if not."),
    OpSpec("MOTR", CreatureHandlers::IntegerRV_MOTR, "", "", categoryCreatures,
           "Returns whether the creature TARG's motor faculty is being "
           "processed or not."),
    OpSpec("SOUL", CreatureHandlers::IntegerRV_SOUL, "i", "facultyId",
           categoryCreatures,
...
OpSpec ourStringRVTable[] = {
    ...
    OpSpec("SUBS", GeneralHandlers::StringRV_SUBS, "sii", "value start count",
           categoryVariables,
           "Slices up a string, returning the substring starting at position "
           "start, with length count.  String indices begin at 1."),
    OpSpec("FACE", CreatureHandlers::StringRV_FACE, "", "", categoryCreatures,
           "Returns the name of the sprite file for the target creature's "
           "face.  Currently automatically gives you the youngest age version "
           "of the gallery but soon will work in the following way: If you set "
           "the parameter to -1 you will get the name of the file the creature "
           "is currently using.  Note that when the creature ages, this file "
           "name will change (the @#GALL@ command could be useful here).  If "
           "you set the parameter to a particular age then the filename "
           "returned will be the gallery that best matches that age.  Use the "
           "@#FACE@ integer rvalue to get the pose number for facing forwards. "
           " See also @#LIMB@."),
    ...
    OpSpec("VOIS", AgentHandlers::StringRV_VOIS, "", "", categorySounds,
           "This returns the voice name for the @#TARG@ agent. (Unless it has "
           "been serialised in :( In which case it returns \"Lozenged\" if the "
           "agent had a voice before the save, or \"\" as normal if the agent "
           "can't speak.)"),
    OpSpec(
    ...
    OpSpec("EAMN", GeneralHandlers::StringRV_EAMN, "s", "previous",
           categoryVariables,
           "Enumerates through @#EAME@ variable names, roughly as @#GAMN@ does "
           "through @#GAME@ variables.  Start and end with an empty string."),
    OpSpec("FVWM", GeneralHandlers::StringRV_FVWM, "s", "name", categoryFiles,
           ...
           "journal file names, etc."),
    OpSpec("HAND", AgentHandlers::StringRV_HAND, "", "", categoryAgents,
           "This returns the name of the hand."),
    OpSpec("UPPA", GeneralHandlers::StringRV_UPPA, "s", "any_old",
           categoryVariables,
           "Converts the given string into all upper case letters."),
    OpSpec("LOWA", GeneralHandlers::StringRV_LOWA, "s", "any_old",
           categoryVariables,
           "Converts the given string into all lower case letters."),
    OpSpec("GALL", AgentHandlers::StringRV_GALL, "", "", categoryAgents,
           "Returns the gallery (sprite file) used by an agent.  This works "
           "for simple and compound agents (using the current @#PART@)."),
    OpSpec("UFOS", GeneralHandlers::StringRV_UFOS, "", "", categoryVariables,
           "This returns the equivalent of \"uname -a\" on compatible systems, "
           "or a description of your operating system on others. This is a "
           "descriptive string and should not be taken as fixed format, or "
           "parseable."),
    OpSpec("MODU", GeneralHandlers::StringRV_MODU, "", "", categoryVariables,
           "Returns a string listed the loaded modules, and the display engine "
           "type.  You can use @#SINS@ to parse this for particular values."),
    OpSpec("LIMB", GeneralHandlers::StringRV_LIMB, "iiiii",
           "body_part genus gender age variant", categoryCreatures,
           "Returns the filename for the specified part of a creature.  If the "
           "exact part isn't present, a 'nearby' file which is on the disk is "
           "returned."),
};
...
OpSpec ourFloatRVTable[] = {
    ...
    OpSpec(
        ...
        "taking the time we would expect them to, which is set by "
        "@#BUZZ@.<br>Factor more than 1 - the engine is running too "
        "slowly.<br>Factor less than 1 - the engine is leaving spare "
        "processing time.<br>This is averaged over the last 10 ticks.<p>Agents "
        "can look at this to adjust the resources they use according to "
        "current spare processing time.  For example, if you have a random "
        "snowflake generator in winter, you could increase the chance of "
        "generation if PACE is low, and decrease the chance if PACE is high.  "
        "When you do this remember that computers will be arbitarily faster in "
        "the future, so you should place an extra upper limit on the number of "
        "snowflakes to stop them filling the whole screen.<p>Note that PACE "
        "only measures the time the engine takes for tick processing, not for "
        "handling requests from external applications, or adding Windows "
        "events to its internal queue.  Because of this, you should aim for a "
        "value which is a bit less than 1.<p>Compare @#RACE@."),
    ...
    OpSpec("FVEL", AgentHandlers::FloatRV_FVEL, "", "", categoryMotion,
           "Get current forwards velocity."),
    OpSpec("SVEL", AgentHandlers::FloatRV_SVEL, "", "", categoryMotion,
           "Get current sideways velocity."),
    OpSpec("AVEL", AgentHandlers::FloatRV_AVEL, "", "", categoryMotion,
           "Get current angular velocity."),
    OpSpec("SPIN", AgentHandlers::FloatRV_SPIN, "", "", categoryMotion,
           "Get current facing angle."),
    OpSpec("ANGL", AgentHandlers::FloatRV_ANGL, "ff", "x y", categoryMotion,
           "Gets the angle (as a fraction of a circle) from TARG's position to "
           "the position specified."),
    OpSpec("VECX", AgentHandlers::FloatRV_VECX, "f", "angle", categoryMotion,
           "Returns a normalised vector for the given angle (X coordinate)."),
    OpSpec("VECY", AgentHandlers::FloatRV_VECY, "f", "angle", categoryMotion,
           "Returns a normalised vector for the given angle (Y coordinate)."),
    OpSpec("ADMP", AgentHandlers::FloatRV_ADMP, "", "", categoryMotion,
           "Get current angular damping.  The value is from 0.0 to 1.0 where "
           "0.0 means no damping, 1.0 maximum."),
    OpSpec("FDMP", AgentHandlers::FloatRV_FDMP, "", "", categoryMotion,
           "Get current forwards damping.  The value is from 0.0 to 1.0 where "
           "0.0 means no damping, 1.0 maximum."),
    OpSpec("SDMP", AgentHandlers::FloatRV_SDMP, "", "", categoryMotion,
           "Get current sideways damping.  The value is from 0.0 to 1.0 where "
           "0.0 means no damping, 1.0 maximum."),
    OpSpec("VARC", AgentHandlers::FloatRV_VARC, "", "", categoryMotion,
           "[not implemented yet]"),
};
...
OpSpec ourAgentRVTable[] = {
    ...
    OpSpec("CARR", AgentHandlers::AgentRV_CARR, "", "", categoryAgents,
           ...
           "there is none."),
    OpSpec(
        "AGNT", AgentHandlers::AgentRV_AGNT, "i", "unique_id", categoryDebug,
        "Given a unique identifier, returns the corresponding agent.  Returns "
        "@#NULL@ if the agent no longer exists.  @#UNID@ extracts the unique "
        "id.  NOTE: This should only be used for external programs to "
        "persistently refer to an agent during a session. Variables can use "
        "@#SETA@ to store agent r-values directly for internal use.  Unique "
        "identifiers can change across saved sessions."),
    ...
    OpSpec("HOTS", AgentHandlers::AgentRV_HOTS, "", "", categoryInput,
           ...
           "transparent pixels.  See also @#HOTP@, which does the same for "
           "compound parts."),
    OpSpec(
    ...
    OpSpec("SEEN", CreatureHandlers::AgentRV_SEEN, "i", "category",
           categoryCreatures,
           "Returns the agent which the creature TARG has currently in mind "
...
OpSpec ourVariableTable[] = {
    ...
    OpSpec("GAME", GeneralHandlers::Variable_GAME, "s", "variable_name",
           ...
           "that variable will be created automatically (with value integer "
           "zero).  Agents, integers, floats and strings can be stored in game "
           "variables.  Variable names are case sensitive.  When a new world "
           "is loaded, all the game variables are cleared.<p>There are some "
           "conventions for the variable names:<br> engine_ for Creatures "
    ...
    OpSpec("FROM", CAOSMachine::Variable_FROM, "", "", categoryAgents,
           "If we're processing a message, this is the @#OWNR@ who sent the "
           "message.  @#NULL@ if the message was sent from an injected script "
           "or an install script.  If the message was sent over the network "
           "using @#NET: WRIT@, then this contains the user id of the sender, "
           "as a string."),
    OpSpec("NAME", AgentHandlers::Variable_NAME, "m", "variable_name",
           categoryVariables,
           "This is a named variable, similar to a @#GAME@ variable, only "
           "local to the target agent.  See also @#MAME@. The \"name\" of the "
           "variable is not limited to strings, but can be anything stored in "
           "a variable. i.e. integer, float, string, even an agent."),
    OpSpec("MAME", AgentHandlers::Variable_MAME, "m", "variable_name",
           categoryVariables,
           "Machine variable version of @#NAME@. Accesses the same variables, "
           "only via @#OWNR@ rather than @#TARG@.  This is the same difference "
           "as between @#MVxx@ and @#OVxx@."),
    OpSpec("EAME", GeneralHandlers::Variable_EAME, "s", "variable_name",
           categoryVariables,
           "Very similar to @#GAME@ variables only they aren't saved or loaded "
           "with the world; they keep their value between worlds for one "
           "session of the engine.  The E in EAME stands for for Engine.  See "
           "the table of engine @#Eame Variables@."),
};

void GeneralPushTables(CAOSDescription &caosDescription) {
  caosDescription.PushTable(idCommandTable, ourCommandTable,
                            sizeof(ourCommandTable) / sizeof(OpSpec));

  caosDescription.PushTable(idIntegerRVTable, ourIntegerRVTable,
                            sizeof(ourIntegerRVTable) / sizeof(OpSpec));
  caosDescription.PushTable(idVariableTable, ourVariableTable,
                            sizeof(ourVariableTable) / sizeof(OpSpec));
  caosDescription.PushTable(idFloatRVTable, ourFloatRVTable,
                            sizeof(ourFloatRVTable) / sizeof(OpSpec));
  caosDescription.PushTable(idStringRVTable, ourStringRVTable,
                            sizeof(ourStringRVTable) / sizeof(OpSpec));
  caosDescription.PushTable(idAgentRVTable, ourAgentRVTable,
                            sizeof(ourAgentRVTable) / sizeof(OpSpec));

  caosDescription.PushTable(AgentHandlers::Command_NEW, idCommandTable,
                            "NEW:", ourSubCommandTable_NEW,
                            sizeof(ourSubCommandTable_NEW) / sizeof(OpSpec));

  caosDescription.PushTable(AgentHandlers::Command_MESG, idCommandTable, "MESG",
                            ourSubCommandTable_MESG,
                            sizeof(ourSubCommandTable_MESG) / sizeof(OpSpec));
  caosDescription.PushTable(CreatureHandlers::Command_STIM, idCommandTable,
                            "STIM", ourSubCommandTable_STIM,
                            sizeof(ourSubCommandTable_STIM) / sizeof(OpSpec));
  caosDescription.PushTable(CreatureHandlers::Command_URGE, idCommandTable,
                            "URGE", ourSubCommandTable_URGE,
                            sizeof(ourSubCommandTable_URGE) / sizeof(OpSpec));
  caosDescription.PushTable(CreatureHandlers::Command_SWAY, idCommandTable,
                            "SWAY", ourSubCommandTable_SWAY,
                            sizeof(ourSubCommandTable_SWAY) / sizeof(OpSpec));
  caosDescription.PushTable(CreatureHandlers::Command_ORDR, idCommandTable,
                            "ORDR", ourSubCommandTable_ORDR,
                            sizeof(ourSubCommandTable_ORDR) / sizeof(OpSpec));
  caosDescription.PushTable(GeneralHandlers::Command_GIDS, idCommandTable,
                            "GIDS", ourSubCommandTable_GIDS,
                            sizeof(ourSubCommandTable_GIDS) / sizeof(OpSpec));

  caosDescription.PushTable(PortHandlers::Command_PRT, idCommandTable,
                            "PRT:", ourSubCommandTable_PRT,
                            sizeof(ourSubCommandTable_PRT) / sizeof(OpSpec));

  caosDescription.PushTable(CompoundHandlers::Command_PAT, idCommandTable,
                            "PAT:", ourSubCommandTable_PAT,
                            sizeof(ourSubCommandTable_PAT) / sizeof(OpSpec));
  caosDescription.PushTable(DebugHandlers::Command_DBG, idCommandTable,
                            "DBG:", ourSubCommandTable_DBG,
                            sizeof(ourSubCommandTable_DBG) / sizeof(OpSpec));
  caosDescription.PushTable(CreatureHandlers::Command_BRN, idCommandTable,
                            "BRN:", ourSubCommandTable_BRN,
                            sizeof(ourSubCommandTable_BRN) / sizeof(OpSpec));

  caosDescription.PushTable(PrayHandlers::Command_PRAY, idCommandTable, "PRAY",
                            ourSubCommandTable_PRAY,
                            sizeof(ourSubCommandTable_PRAY) / sizeof(OpSpec));
  caosDescription.PushTable(PrayHandlers::IntegerRV_PRAY, idIntegerRVTable,
                            "PRAY", ourSubIntegerRVTable_PRAY,
                            sizeof(ourSubIntegerRVTable_PRAY) / sizeof(OpSpec));
  caosDescription.PushTable(PrayHandlers::StringRV_PRAY, idStringRVTable,
                            "PRAY", ourSubStringRVTable_PRAY,
                            sizeof(ourSubStringRVTable_PRAY) / sizeof(OpSpec));

  caosDescription.PushTable(CreatureHandlers::Command_GENE, idCommandTable,
                            "GENE", ourSubCommandTable_GENE,
                            sizeof(ourSubCommandTable_GENE) / sizeof(OpSpec));
  caosDescription.PushTable(GeneralHandlers::Command_FILE, idCommandTable,
                            "FILE", ourSubCommandTable_FILE,
                            sizeof(ourSubCommandTable_FILE) / sizeof(OpSpec));

  caosDescription.PushTable(HistoryHandlers::Command_HIST, idCommandTable,
                            "HIST", ourSubCommandTable_HIST,
                            sizeof(ourSubCommandTable_HIST) / sizeof(OpSpec));
  caosDescription.PushTable(HistoryHandlers::IntegerRV_HIST, idIntegerRVTable,
                            "HIST", ourSubIntegerRVTable_HIST,
                            sizeof(ourSubIntegerRVTable_HIST) / sizeof(OpSpec));
  caosDescription.PushTable(HistoryHandlers::StringRV_HIST, idStringRVTable,
                            "HIST", ourSubStringRVTable_HIST,
                            sizeof(ourSubStringRVTable_HIST) / sizeof(OpSpec));

  caosDescription.PushTable(PortHandlers::IntegerRV_PRT, idIntegerRVTable,
                            "PRT:", ourSubIntegerRVTable_PRT,
                            sizeof(ourSubIntegerRVTable_PRT) / sizeof(OpSpec));
  caosDescription.PushTable(PortHandlers::StringRV_PRT, idStringRVTable,
                            "PRT:", ourSubStringRVTable_PRT,
                            sizeof(ourSubStringRVTable_PRT) / sizeof(OpSpec));
  caosDescription.PushTable(PortHandlers::AgentRV_PRT, idAgentRVTable,
                            "PRT:", ourSubAgentRVTable_PRT,
                            sizeof(ourSubAgentRVTable_PRT) / sizeof(OpSpec));

  caosDescription.PushTable(SoundHandlers::Command_CD, idCommandTable, "_CD_",
                            ourSubCommandTable_CD,
                            sizeof(ourSubCommandTable_CD) / sizeof(OpSpec));
  caosDescription.PushTable(SoundHandlers::IntegerRV_CD, idIntegerRVTable,
                            "_CD_", ourSubIntegerRVTable_CD,
                            sizeof(ourSubIntegerRVTable_CD) / sizeof(OpSpec));
  ...
  caosDescription.SetCategoryText(std::vector<std::string>(
      ourCategoryText,
...
int dummyGeneralPushTables =
    CAOSDescription::AddTableRegisterFunction(&GeneralPushTables);
