...
public:
  ...
  public:
    Exception(const char *msg) : BasicException(msg) {}
  };
...
#pragma warning(default : 4251)
  ...
  //				containing pointers or any class whose
...
CreaturesArchive &operator<<(CreaturesArchive &ar, const std::list<C> &value) {
  ar << (uint32)(value.size());
  for (typename std::list<C>::const_iterator i = value.begin();
       i != value.end(); ++i) {
    ar << *i;
  }
  return ar;
...
CreaturesArchive &operator<<(CreaturesArchive &ar, const std::deque<C> &value) {
  ar << (uint32)(value.size());
  for (typename std::deque<C>::const_iterator i = value.begin();
       i != value.end(); ++i) {
    ar << *i;
  }
  return ar;
