...
FlightRecorder::FlightRecorder() {
  ...
  // is no good for this, as it hasn't been read yet!).  We look for this
  // file...
...
void FlightRecorder::Log(FlightCategory category, const char *fmt, ...) {
  ...
  //     ErrorMessageHandler::ErrorMessageFooter();
...
void FlightRecorder::SetCategories(uint32 enablemask) {
  ...
  // And even when the config file is read, if logtastic is there, log startup
  // and shutdown
  if (FileExists("logtastic")) {
