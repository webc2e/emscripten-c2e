...
public:
	...
	bool SearchForAdjacentRoom
		(const Vector2D& position, 
		 const int direction,
		 int& roomID);

	bool FindSafeAgentLocation
	...
	bool CanPointSeePoint
		(const Vector2D& start, 
		 const Vector2D& end, 
		 const int minDoorPermiability, 
		 bool& canSee);

	bool IsAgentLocationValidInRoomSystem
		(const Vector2D& position, 
		 const float width, 
		 const float height,
		 const int minDoorPermiability);

	bool IsAgentLocationValidInRectangle
		(const Vector2D& position, 
		 const float width, 
		 const float height,
		 const float left,
	...
	bool IsCreatureLocationValidInRoomSystem
		(const Vector2D& footLeft,
		 const Vector2D& footRight,
		 const float yMin,
		 const int minDoorPermiability);

	bool IsCreatureLocationValidInRectangle
		(const Vector2D& footLeft,
		 const Vector2D& footRight,
		 const float yMin,
		 const float left,
...
void ShiftCreatureAlongFloor
	...
	bool GetMetaRoomTrack
		(const int metaRoomID, std::string& track);

	bool SetMetaRoomTrack
		(const int metaRoomID, std::string& track);
	...
	int GetMetaRoomCount(void);

	bool SetCurrentMetaRoom
		(const int metaRoomID);

	int GetCurrentMetaRoom(void);

	bool GetMetaRoomDefaultCameraLocation
		(const int metaRoomID, long& positionX, long& positionY);

	bool SetMetaRoomDefaultCameraLocation
	...
	bool SetCurrentBackground
		(const int metaRoomID, std::string background);

	bool GetCurrentBackground
		(const int metaRoomID, std::string & background);
	...
	bool GetRoomPropertyMinusMyContribution( const AgentHandle &handle, float &value );

	bool GetRoomPropertyChange( int roomID, int caIndex, float &value );

	bool GetProjectedRoomProperty( int roomID, int caIndex, float &value );
	...
	bool WhichDirectionToFollowCA(int currentRoomID, int caIndex, int &mapDirection, bool approachNotRetreat);
	...
	bool GetRoomIDWithHighestCA( int roomID, int caIndex, bool includeUpDown, int &roomIDHighest );

	bool GetRoomIDWithLowestCA( int roomID, int caIndex, bool includeUpDown, int &roomIDLowest );
	...
	void AlterCAEmission( Room *room, int caIndex, float difference,
...
private:
	...
	bool IsPointBelowCeilingDoors
		(const Vector2D& position, Room* room);

	bool IsPointAboveFloorDoors
		(const Vector2D& position, Room* room);

	void GetGridIndexesForPoint
		(const Vector2D& position, int& squareX, int& squareY);

	bool GetRoomInformationForPoint
		(const Vector2D& position, 
		 int roomIDList[4], 
		 Room* roomList[4],
		 int& roomCount,
	...
	void CalculateCollisionVelocityAndElapsedTime
		(const Vector2D& velocity, 
	...
	void DoesPathReachRoomBoundary
		(Room* room, 
		 const Vector2D& start, 
		 const Vector2D& end, 
		 const Vector2D& delta, 
	...
	bool AreLinesValidInRoomSystem
		(Vector2D starts[4], 
		 Vector2D ends[4],
		 const Vector2D& positionMin,
		 const Vector2D& positionMax,
	...
	void GetNearbyFloorInformation
	(const Vector2D& position,
	 Room** floorRooms,
	 int& floorRoomCount);


bool IsRegionBlockedByVerticalWall
	(Room** floorRooms,
	 int floorRoomCount,
	 const int minDoorPermiability,
...
void SnapFeetToFloor
	...
	bool GetDoorBetweenRooms
		(const int id1, const int id2, Door*& door);

	bool GetLinkBetweenRooms
