...
//
//
//				The camera keeps two backgrounds one incoming and one outgoing
//				so that if a meta room change is requested the the incoming
//				 background is loaded in and the outgoing background is kept
//				in case there is an error in loading and we need to go back
//				to it.  Also the outgoing background serves as a cache so that 
//				if the last seen meta room in case the user goes back to it
//				immediately.  And finally it allows you to use fading effects
//				between the two backgrounds when going from meta room to meta
//				room.
//				
//
			...
			)
{
	...
	#else
		if(myScreen)
			myScreen->Create(defaultBackground,
							topLeftXofBackground,
							topLeftYofBackground,
							myWorldPosition);
	#endif
	...
	)
{
	...
	//	_ASSERT(false);
		return true;
	#else
		myScreen->Create(myBackground1FilePath.GetFullPath(),
							bounds,
							myWorldPosition);
	#endif
			...
			bool bCentre)
{
	...
	if(!FileExists(myBackground1FilePath.GetFullPath().c_str()))
	{
		...
		myPicture = new C2DLayer(myScreen,myBackground1FilePath.GetFullPath().c_str());
		myScreen->Flip();
	...
	if(useTopLeftOnly)
	{
		...
		ok = myScreen->Create(myBackground1FilePath.GetFullPath(),
									bounds.top,
								bounds.left,
								myWorldPosition);
	}
...
bool Camera::Pan()
{
	...
	if (myPanVector.GetX()<0)	// Moving left
	{
		...
		if (myPanVector.GetX() >= -acceleration || myPanVector.GetX() >= myPanVelocity.GetX())
		{
			// Finish last step
	...
	{
		// Stop if we're very near or are going to overshoot
		if (myPanVector.GetY() >= -acceleration || myPanVector.GetY() >= myPanVelocity.GetY())
		{
			// Finish last step
...
void Camera::NormaliseWorldCoordinates(int32& x, int32& y)
{
	...
	// my bitmap
	if(x-left + displayWidth> 
		pixelWidth)
	{
		x = pixelWidth - displayWidth +
			left;
	}

	if(y-top + displayHeight>  
		pixelHeight)
	{
		y = pixelHeight;
		y = pixelHeight - displayHeight
			+top ;
	}
...
bool Camera::KeepUpThenWithMouse()
{
	...
	{	
		...
		{
			int32 mouseActiveBorderSizeTop = theApp.GetEameVar("engine_nudge_border_t").GetInteger();
			int32 mouseActiveBorderSizeBottom = theApp.GetEameVar("engine_nudge_border_b").GetInteger();
			int32 mouseActiveBorderSizeLeft = theApp.GetEameVar("engine_nudge_border_l").GetInteger();
			int32 mouseActiveBorderSizeRight = theApp.GetEameVar("engine_nudge_border_r").GetInteger();

			// mouse
			if (x <= myWorldPosition.GetX() + mouseActiveBorderSizeLeft)
				panx = -1;
			else if (x >= myWorldPosition.GetX() +  (myDisplayRect.right - myDisplayRect.left) - mouseActiveBorderSizeRight)
				panx = 1;
			if (y <= myWorldPosition.GetY() + mouseActiveBorderSizeTop)
				pany = -1;
			else if (y >= myWorldPosition.GetY() + (myDisplayRect.bottom - myDisplayRect.top) - mouseActiveBorderSizeBottom)
				pany = 1;
		}
...
bool Camera::Visible(RECT& rect)
{
	...
	// move to sprite
#ifdef C2D_DIRECT_DISPLAY_LIB
	return ((rect.right >= myViewArea.left) &&
			(rect.left <= myViewArea.right) &&
			(rect.top <= myViewArea.bottom) &&
			(rect.bottom >= myViewArea.top));
	#else
...
//********
		...
		myScreen->Create( 0, pos,backgroundName,topLeft,myPixelWidth,myPixelHeight);
...
//**********
		...
		myScreen->Create( 1, pos,backgroundName,topLeft,myPixelWidth,myPixelHeight);
...
void GenericWorldToScreen(IntOrFloat& x, IntOrFloat& y, IntOrFloat viewAreaLeft, IntOrFloat viewAreaTop)
{
	x -= viewAreaLeft;
	y -= viewAreaTop;
}
...
template <class IntOrFloat>
void GenericScreenToWorld(IntOrFloat& x, IntOrFloat& y, IntOrFloat viewAreaLeft, IntOrFloat viewAreaTop)
{
	x += viewAreaLeft;
	y += viewAreaTop;
}
...
bool Camera::GetLoading()
{
	return myLoadingFlag;
}
...
#ifdef C2D_DIRECT_DISPLAY_LIB
void Camera::DrawLineToScreen(int x1,
					int y1,
					int x2,
					int y2 ,	 
					uint8 lineColourRed,
					...
					uint8 lineColourBlue)
{
	...
	{
		uint32 lineColour = 0;
		lineColour = (((lineColourRed) & 0xf8) << 8) | (((lineColourGreen) & 0xfc) << 3)|(((lineColourBlue) & 0xf8) >> 3); 
