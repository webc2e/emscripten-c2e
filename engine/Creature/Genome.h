...
// Offsets into the Header Gene (gene(0)). These are fields that get written to
// during conception
enum headeroffsets {
...
const int LINKAGE =
    50; // AVERAGE # genes between crossover points. (range = 1 to
...
const int CUTERRORRATE =
    80; // Probability that a crossover will lead to a gene duplication
...
const int MUTATIONRATE =
    4800; // Probability that a codon will mutate (1/n per codon)

// Gene TYPE numbers
enum {
  BRAINGENE = 0,    // brain genes (CBrain class)
  BIOCHEMISTRYGENE, // Physiology genes (emitter, receptors, reactions &
                    // half-lives)
...
//		REGION_ARMS,2,							//
//set arms to use variant 2
// NOTE: if no gene exists for arms &/or tail, then no such body region will be
// created. Otherwise, absence of a gene means "use variant 0"
enum bodyregions { // regions of body that are controlled by a single gene
  //+++ The first 2 regions are remmed out until articulated head parts are
  // needed
...
// Gene-switching flags. Pass one of these to GetGeneType() to override the
// default behaviour of switching genes on when their time is right
enum geneswitchoverrides {
  ...
  // e.g. to re-read such things as APPEARANCE or HEADER genes, during later
  // scans
  SWITCH_ALWAYS,
  // switch on if myAge=0, regardless of what switch-on time is stored in the
  // gene header.
  // This is useful to force eg. LOBE genes to switch on during Embryology, even
  // if
...
public:
  const char *what() { return myMessage.c_str(); }
...
class Genome : public PersistentObject {
  CREATURES_DECLARE_SERIAL(
      Genome) // serialisable so that exported creatures save their genomes
public:
  inline int GetAge() { return myAge; }
  inline void SetAge(int age) { myAge = age; }
  inline int GetSex() { return mySex; }
  inline std::string GetMoniker() { return myMoniker; }

  inline void SetMoniker(const std::string &newMoniker) {
    myMoniker = newMoniker;
  }

  inline byte *GetGenePointer() { return myGenePointer; }
  inline void AdjustGenePointerBy(int i) { myGenePointer += i; }
...
protected:
  ...
  int myVariant; // (new for C2e) express genes of this variant only. (0=all,
                 // 1-NUM_BEHAVIOUR_VARIANTS=behav)
...
public:
  ...
  Genome(); // null constr
  Genome(GenomeStore &store, int storeIndex,
  ...
  bool GetGeneType( // Find the NEXT gene of the given type & subtype.
      int type, int subtype, int numsubs,
      int flag = SWITCH_AGE, // special switch-on condition (dflt is switch if
                             // correct age)
      int endtype = INVALIDGENE, int endSubType = INVALIDSUBGENE,
      int otherEndType = INVALIDGENE);

  int CountGeneType( // Count the number of genes of the given type
      int type, int subtype, int numsubs);

  int GetCodon(int min,
               int max); // Extract a codon from the NEXT position in a gene
  ...
  inline float GetSignedFloat() {
    return (((float)GetCodon(0, 248)) / 124.0f) - 1.0f;
  }
  inline int GetInt() {
...
private:
  void CrossLoop(Genome *mum, Genome *dad, byte MumChanceOfMutation,
                 byte MumDegreeOfMutation, byte DadChanceOfMutation,
                 byte DadDegreeOfMutation);

  bool GetStart(); // Find the NEXT gene start marker. Set myGenePointer to
                   // point to its TYPE codon
  bool
  TimeToSwitchOn(byte switchontime, // is it Ok for this gene to switch on now?
                 int flag);
  ...
  void
  CopyGene(Genome *src, // Copy next gene from src genome to my genome, mutating
...
MUTATION
Mutations must ONLY happen inside the data area of a gene, never in the start
marker or the TYPE or SUBTYPE entries. Therefore, the mutate() function must
scan the genome and count how many genes there are, then pick one of those genes
at random and find its length, then pick a data codon within the gene and mutate
it. This ensures the integrity of the genome. New genes can still be added to
the system by a combination of cutting error and mutation, but no existing genes
can have their type changed, or be cut in two.

CROSSING OVER
Crossing over must ONLY happen at gene boundaries. Also, a +/-1 gene cutting
error should be allowed. Thus genes can be deleted, and new genes added during
crossing over. When a gene is replicated, it has no significant effect until and
unless one of the pair gets mutated. It could speed things up if I automatically
introduce a mutation in any duplicated genes.


DEVELOPMENT AND SENESCENCE
Give each gene a 'switching on time'. Then run the gene expressor several times
during the life of a creature (at startup, maybe). This allows me to add genes
that don't switch on until, say, adolescence or old age, thus allowing for
sexual maturation and senescence. (Note: switch-on times are mutable, allowing
evolution of different life histories). It would even be feasible (just in case)
for me to allow child and adult 'appearance' genes!

Also, define body poses using genes - then babies, adolescents and adults could
have different walking postures, etc. (could babies crawl, or otherwise be
unable to reach high things?). In principle, this would also allow young & old
body sprites!?

INSTINCT genes can also be switched on. This allows maturity to control eg.
sexual behaviour (attraction to opposite sex), language development, etc.

MUTATIONS
Have a single FLAGS byte in the gene header. Bits in here determine whether the
gene may be duplicated, excised and/or mutated during crossing-over
...
- gene ID# (ignore this here, it is used by the Gene Editor so that it can
associate gene captions & annotations with the correct original or cloned gene)
- Generation# (Used by the gene editor to detect genes that have been cloned by
cutting errors. This program should increment this value in any cloned
(duplicated) gene)
- Switch-on-time (Used to determine whether this gene should be expressed now.
Zero means express only at conception; other values allow genes to be switched
on later in life)
- mutability flags (Used to control whether a gene can be duplicated or lost
during crossing-over and whether it may become mutated) After this come the
gene-specific data.
