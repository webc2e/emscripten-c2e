...
public:
  ...
  enum {
    LEARNING_DELAY = 1,
    NORMAL_DELAY_OFFSET = 4,
    INTERMEDIATE_DELAY_OFFSET = 6,
    REPLY_TO_CREATURE_DELAY_OFFSET = 9
  };
  ...
  // Returns:		None
  // ---------------------------------------------------------------------------
  void Init(); // initialise vocabulary (and voice)

  // ---------------------------------------------------------------------------
  // Function:	SetWord
  // Description:	Sets the word the creature is to know, does not know how to
  // say
  ...
  // ---------------------------------------------------------------------------
  // Function:	KnownWord
  // Description:	returns whether the word is known by the creature even if
  // it
  ...
  //				either what it is doing, its need, or opinion of
  //others
  // Arguments:	None
  // Returns:		None
  // ---------------------------------------------------------------------------
  ...
  // Returns:		None
  // ---------------------------------------------------------------------------
  void ExpressNeed(int requestedDriveToExpress = NONE,
                   int delay = NORMAL_DELAY_OFFSET);

  // ---------------------------------------------------------------------------
  // Function:	ExpressOpinion
  // Description:	ques sentence about creatures opinion about a another
  // creature
  //				or the pointer
  // Arguments:	creature or pointer to talk about
  // Returns:		None
  // ---------------------------------------------------------------------------
  void ExpressOpinion(AgentHandle creatureOrPointer,
                      int delay = NORMAL_DELAY_OFFSET);
  ...
  // Returns:		None
  // ---------------------------------------------------------------------------
  void StackSentence(std::string sentence, bool shout = false,
                     int delay = NORMAL_DELAY_OFFSET, int verb = NONE,
                     int noun = NONE);
  ...
  // Returns:		None
  // ---------------------------------------------------------------------------
  void HearSentence(AgentHandle speakerAgent, std::string sentence,
  ...
  bool ParseSentence(AgentHandle speakerAgent, std::string stentenceSpoken,
                     std::string &syntax, int wordsHeard[noOfTypes],
                     int learnVerb, int learnNoun);

  // ---------------------------------------------------------------------------
  // Function:	GetWordType
  // Description:	matches first part of sentence agains a word the creature
  // can
  //				say	or a word it is learning to say or another creatres
  //name
  // Arguments:	sentence to look in, fills in - whether
  //				the word was understood type and id of word and the
  //size 				of the matched word
  // Returns:		If match was found
  // ---------------------------------------------------------------------------
  bool GetWordType(std::string wordToMatch, bool &wordUnderstood, int &wordType,
                   int &wordHeard, int &wordSize);
  ...
  // ---------------------------------------------------------------------------
  std::string TranslateToBase(std::string syntax);
  ...
  // Returns:		translated syntax string
  // ---------------------------------------------------------------------------
  std::string TranslateToLocal(std::string syntax);
...
protected:
  struct Command {
    int verbToNudge;
    int nounToNudge;
    float amountToNudgeVerb;
    float amountToNudgeNoun;
  };
  Command
      myLastSpokenCommandGivenToMeWhichIHaventBeenAbleToDoYetBecauseImNotLookingAtTheRightNoun;
  static Command GetNullCommand();
