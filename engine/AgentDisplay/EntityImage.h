...
typedef std::map<std::string,C2DAniStrip*>::iterator C2DANISTRIPMAP_ITERATOR;
...
// ----------------------------------------------------------------------
	...
	EntityImage(FilePath galleryName, 
			int32 numimages,
		int32 plane,//plane
...
// ----------------------------------------------------------------------
	...
	bool IsMirrored();
	bool IsVisible(){return myVisibleFlag;}
...
// ----------------------------------------------------------------------
	...
	C2DSprite* GetSpritePtr() 
	{
		return mySprite;
	}

	C2DAniStrip* GetAnimStripPtr() 
	{
		return myAniStrip;
	}

	void ClearSpritePtr(){mySprite = NULL;}
	void DrawFlippedVertically(bool flip);
...
// ----------------------------------------------------------------------
	...
	void UpdateOverlayMirroring(uint32 layer, bool yesOrNo );
#endif

#ifdef C2D_DIRECT_DISPLAY_LIB

	virtual void CreateAnimStrip(int blockWidth, int blockHeight);
	static void RemoveAllAnimStrips();
...
// ----------------------------------------------------------------------
	...
	FilePath GetFilePath(){return myFilePath;}

	uint32 GetGalleryBitmapCount();

	bool RemoveGallery();

	void RecolourGallery(const uint16* tintTable);
...
// ----------------------------------------------------------------------
	...
	void SetBlockWidth(uint32 width){myBlockWidth = width;}
	void SetBlockHeight(uint32 h){myBlockHeight = h;}

	int GetBlockWidth(){return myBlockWidth;}
	int GetBlockHeight(){return myBlockHeight;}

	virtual void Scale(float scaleby, bool onOrOff);
	void DrawAlphaBlended(int alphamask, bool onOrOff );
	...
	void DrawOutlineAroundImage(int red, int green, int blue);
#endif
...
protected:
	...
	#ifdef C2D_DIRECT_DISPLAY_LIB
		C2DSprite*			mySprite;
		C2DAniStrip*		myAniStrip;
		static std::map<std::string,C2DAniStrip*> ourAnimationStrips;
	#else
		Sprite*				mySprite;
	#endif
	...
	FilePath myFilePath;
	FilePath myOverlayFilePath;

	uint32 myBlockWidth;
	uint32 myBlockHeight;
