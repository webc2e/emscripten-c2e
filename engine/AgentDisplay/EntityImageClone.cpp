...
ClonedEntityImage::ClonedEntityImage() { myReadArchivedGalleryFlag = false; }
...
ClonedEntityImage::ClonedEntityImage(FilePath const &galleryName,
                                     ...
                                     uint32 numImages, uint32 currentimage) {
...
ClonedEntityImage::ClonedEntityImage(int32 plane,  // plane
                                     int32 worldx, // world x
                                     int32 worldy, // world y
...
bool ClonedEntityImage::SetGallery(FilePath const &galleryName,
                                   uint32 baseImage, uint32 numImages /*= 0*/,
                                   bool reset /*= true*/) {
#ifdef C2D_DIRECT_DISPLAY_LIB
  ...
  myAniStrip = new C2DAniStrip(theMainView.GetSpritePtr(),
                               galleryName.GetFullPath().c_str(), myBlockWidth,
                               myBlockHeight, myNumberOfImages);
...
#ifdef C2D_DIRECT_DISPLAY_LIB
void ClonedEntityImage::SetFontAttributes(int width, int height, uint32 colour,
                                          uint32 type) {
  myFontWidth = width;
  myFontHeight = height;
...
int ClonedEntityImage::SelectFontType(std::string const &fontName, int r, int g,
                                      int b, int rot, int swap) {
#ifdef C2D_DIRECT_DISPLAY_LIB
  if (myAniStrip) {
...
void ClonedEntityImage::DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
  ...
  mySprite->DrawLineToCurrentFrame(x1, y1, x2, y2, lineColourRed,
                                   lineColourGreen, lineColourBlue);

#else

  mySprite->DrawLine(x1, y1, x2, y2, lineColourRed, lineColourGreen,
                     lineColourBlue);
#endif
}
...
void ClonedEntityImage::DrawSprite(Position &whereToDraw,
                                   ...
                                   int32 spriteIndex, int32 baseimage,
                                   int32 numimages) {
#ifdef C2D_DIRECT_DISPLAY_LIB
//	_ASSERT(false);
// this function is used for text in C2e and is so not
...
#else
  Gallery *gallery =
      new Gallery(galleryName.GetFullPath(), baseimage, numimages);

  Bitmap *bitmap = gallery->GetBitmap(spriteIndex);

  if (bitmap) {
    ClonedEntityImage::DrawSprite(whereToDraw, bitmap);
  }
  ...
  for (int i = 0; i < mySprite->GetBitmapCount(); i++) {
    ...
    archive.Write((void *)mySprite->GetBitmap(i)->GetData(),
                  mySprite->GetBitmap(i)->GetWidth() *
                      mySprite->GetBitmap(i)->GetHeight() * sizeof(uint16));
  }
#endif
  return true;
}
...
bool ClonedEntityImage::Read(CreaturesArchive &archive) {
  ...
  if (version >= 13) {
    ...
    mySprite->CreateGalleryForCloning(myFilePath.GetFullPath(), baseImage,
                                      pixelFormat, numBitmaps);
...
void ClonedEntityImage::RefreshAllAnimStrips() {
  for (C2DANISTRIPVECTOR_ITERATOR it = ourClonedAnimationStrips.begin();
       it != ourClonedAnimationStrips.end(); it++) {

    (*it)->Create(theMainView.GetSpritePtr(), NULL, -1, -1, -1);
...
void ClonedEntityImage::DeleteAnimStrip() {
  ...
  // ourClonedAnimationStrips.find(myFilePath.GetFullPath().c_str());
  for (C2DANISTRIPVECTOR_ITERATOR it = ourClonedAnimationStrips.begin();
       it != ourClonedAnimationStrips.end(); it++) {
    if ((*it) == myAniStrip) {
