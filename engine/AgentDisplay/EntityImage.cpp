...
// Class:		EntityImage
// Purpose:		This class is the interface to the rest of C2e.  This
// class
//				is closely based on the C2 entity and will
// evolve as required.
//
//
//
...
//
//	Entities are the things that get redrawn when a rectangle is
// invalidated. 	A library of pointers to all extant entities is stored
// in EntityLib[] 	for this purpose.
//
//	Entities are ALWAYS encapsulated in Objects of one kind or another.
// Thus, 	although each entity contains its own X&Y members, these get set
// as 	appropriate by the Object which owns them, and don't need to be
...
//
//
//	Entity				Simplest form of entity, instantiated
// for use 	|					within SimpleObjects and
// CompoundObjects.
//	|					Also serves as base class for
// articulated
// entities, 	| 					for encapsulation within
// Creatures.
//	| 					Whereas a Sprite uniquely
// represents a given bitmap, 	| 					several
// Entities
// can refer to the same Sprite 	| - an Entity represents the simplest
// individual
//	| 					object in the world; several
// Entities
// may LOOK 	| 					the same, but ARE
// different.
//	|
//	|
//	BodyPart
//	| |
//	| Body				Parts of articulated objects. A Limb is
// connected to
//	| 					one or no daughter Limbs (eg. a
// thigh Limb connects 	| 					to a shin Limb);
// A Body is similar, but connects 	Limb                to several Limb
// chains. The
// BodyPart class contains 						methods & data
// that are common to both types Body & Limb classes contain pointers to
// BodyData or LimbData classes respectively.
//
//						The BodyPart Entities contain
// pointers to
// LimbData 						or BodyData structs, which
// contain the
// XY 						information that is common to
// all BodyParts with 						the same
// appearance.
//
//
//
...
//							Changed Class and file
// names from
// Object to EntityImage.
...
EntityImage::EntityImage()
    : myCurrentOverlayIndex(-1), myReadArchivedGalleryFlag(1),
      myCurrentBaseImageIndex(0), myAbsoluteBaseImage(0),
      myCurrentImageIndex(0), mySavedImageIndexState(0), myFrameRate(1),
      myFrameCount(0), myCurrentWidth(0), myCurrentHeight(0),
      myVisibleFlag(true), mySprite(0), mySpriteIsCameraShy(false),
      myDrawAlphaFlag(false), myAlphaIntensity(0), myScaledFlag(false),
      myBlockWidth(0), myBlockHeight(0) {
#ifdef C2D_DIRECT_DISPLAY_LIB
  myAniStrip = NULL;
#else
  mySprite = new Sprite();
...
EntityImage::EntityImage(FilePath galleryName, int32 numimages, int32 plane,
                         int32 worldx, int32 worldy, uint32 baseimage,
                         uint32 currentimage)
    : myNumberOfImages(numimages), myCurrentOverlayIndex(-1),
      myReadArchivedGalleryFlag(1), myFrameRate(1), myFrameCount(0),
      myCurrentWidth(0), myCurrentHeight(0), myVisibleFlag(true),
      mySpriteIsCameraShy(false), mySprite(0), myFilePath(galleryName),
      myDrawAlphaFlag(false), myAlphaIntensity(0), myScaledFlag(false),
      myBlockWidth(0), myBlockHeight(0) {
  ...
  if (myNumberOfImages != 0 && baseimage > myNumberOfImages) {
    ...
    std::string string =
        ErrorMessageHandler::Format(theDisplayErrorTag, (int)sidInvalidBasePose,
  ...
  if (!galleryName.empty()) {
    ...
    if (!SetGallery(galleryName, baseimage)) {
      ...
      std::string string = ErrorMessageHandler::Format(
          theDisplayErrorTag, (int)sidNoGalleryCreated,
          std::string("EntityImage::EntityImage"),
          galleryName.GetFullPath().c_str());
...
void EntityImage::RemoveAllAnimStrips() {
  for (C2DANISTRIPMAP_ITERATOR it = ourAnimationStrips.begin();
       it != ourAnimationStrips.end(); it++) {
    delete (*it).second;
    (*it).second = NULL;
...
void EntityImage::RefreshAllAnimStrips() {
  for (C2DANISTRIPMAP_ITERATOR it = ourAnimationStrips.begin();
    ...
    (*it).second->Create(theMainView.GetSpritePtr(), NULL, -1, -1, -1);
...
void EntityImage::Init(int32 plane,      // plane
                       ...
                       uint32 currentimage, uint32 numimages) {
#ifdef C2D_DIRECT_DISPLAY_LIB
...
void EntityImage::DeleteAnimStrip() {
  C2DANISTRIPMAP_ITERATOR it =
      ourAnimationStrips.find(myFilePath.GetFullPath().c_str());
...
// start from numImages - the number of images for this sprite
//
//
// Returns:     None
//
...
//
// ----------------------------------------------------------------------
bool EntityImage::SetGallery(FilePath const &galleryName, uint32 baseimage,
                             uint32 numImages /*= 0*/, bool reset /*=true*/) {
#ifdef C2D_DIRECT_DISPLAY_LIB
  ...
  C2DANISTRIPMAP_ITERATOR it =
      ourAnimationStrips.find(galleryName.GetFullPath().c_str());
...
// replace
// their 				normal image by one from an overlay
// gallery.
// The overlay 				gallery is optional and must be created
...
bool EntityImage::SetCurrentBitmap() {
  ...
  if (mySprite) {
    ok = mySprite->SetBitmap(myCurrentImageIndex, 0, Sprite::PRIMARY_GALLERY);
...
//_ASSERT(false);
#else
  for (uint32 i = myAbsoluteBaseImage;
       i < myAbsoluteBaseImage + myNumberOfImages; i++) {
...
bool EntityImage::Visible(const RECT &test) {
  ...
  return (((dl > (test.right - test.left)) || (test.bottom < GetWorldY()) ||
           (dr < 0) || (test.top > (GetWorldY() + GetHeight())))
              ? 0
              : 1);
}
...
//								if this is -1
//then use the
// current image 				x - x coordinate of the pixel to
// test y - y coordinate of the pixel to test
//
// Returns:     true if the pixel at the given coordinates is transparent
//				false otherwise
//
// ----------------------------------------------------------------------
bool EntityImage::IsPixelTransparent(uint32 x, uint32 y,
                                     int32 imageIndex /* =-1*/) {
  if (imageIndex == -1)
...
bool EntityImage::Write(CreaturesArchive &archive) const {
  ...
  if (myBlockWidth > 0 && myBlockHeight > 0) {
    if (!mySprite) {
      std::string message =
          "No sprite but we have a block width/height.\nFilename: " +
          myFilePath.GetFileName();
      throw EntityImageException(message, __LINE__);
    }
  } else {
    if (mySprite) {
      std::string message =
          "No block width/height but we have a sprite.\nFilename: " +
          myFilePath.GetFileName();
      throw EntityImageException(message, __LINE__);
    }
  }
...
bool EntityImage::Read(CreaturesArchive &archive) {
  ...
  if (version >= 3) {
    ...
    if (myFilePath.empty()) {
      std::string string = ErrorMessageHandler::Format(
          theDisplayErrorTag, (int)sidGalleryNotSpecified,
...
#ifdef C2D_DIRECT_DISPLAY_LIB
    // for now in the direct display engine cloned entities don't store their
    // data
    ...
    if (myReadArchivedGalleryFlag) {
...
#endif
      {
        ...
        if (!mySprite) {
          std::string message =
              "Failed to make a sprite during loading\nFilename: " +
              myFilePath.GetFileName();
          throw EntityImageException(message, __LINE__);
        }
#endif
    ...
    if (version >= 22) {
      ...
      if (spritesaved) {
        if (!mySprite) {
          std::string message =
              "Inconsistent state while loading sprite\nFilename: " +
              myFilePath.GetFileName();
          throw EntityImageException(message, __LINE__);
        }
        archive >> mySprite->m_BltType;
...
bool EntityImage::AnimOver() { return (myAnimLength == 0); }
...
void EntityImage::DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
  ...
  if (mySprite) {
    ...
    mySprite->AddLine(wx1, wx2, wy1, wy2, lineColourRed, lineColourGreen,
                      lineColourBlue);
  }
#else

  Line *line = theMainView.NewLine(
      x1, y1, x2, y2, lineColourRed, lineColourGreen, lineColourBlue, stippleon,
      stippleoff, stipplestartAt, theApp.GetLinePlane());
...
void EntityImage::ReloadGallery(FilePath const &galleryName, uint32 baseimage,
                                uint32 numImages /*= 0*/) {
  myCurrentBaseImageIndex = baseimage;
...
void EntityImage::PrepareForReloadingGallery() { Unlink(); }
...
uint16 EntityImage::GetSpriteRotationAngle() { return mySprite->m_Angle; }
...
bool EntityImage::ValidatePose(int pose) {
  ...
  if ((myCurrentBaseImageIndex + pose) >=
      myAbsoluteBaseImage + myNumberOfImages)
    return false;
...
void EntityImage::DrawToNewWidthAndHeight(int newwidth, int newHeight,
                                          bool on) {
  if (!mySprite)
    return;
