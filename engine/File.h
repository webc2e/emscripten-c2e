...
public:
  ...
  enum FilePosition { Start = SEEK_SET, Current = SEEK_CUR, End = SEEK_END };
#endif
...
#ifdef _WIN32
  uint32 GetSize(void) { return myLength; }
#else
  uint32 GetSize(void);
#endif
  ...
  //				from - from the end begining or current position in
  //the 						file.
  // Description:
  // ----------------------------------------------------------------------
  ...
  bool Valid() { return (myDiskFileHandle != INVALID_HANDLE_VALUE); }

  oshandle GetHandle() { return myDiskFileHandle; }
