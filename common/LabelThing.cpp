...
CLabelThing::CLabelThing(UINT uShowDelay, UINT uHideDelay)
    : m_uTimerID(0), m_uShowDelay(uShowDelay), m_uHideDelay(uHideDelay),
      m_bVisible(false) {}
...
BOOL CLabelThing::Create(CWnd *pParentWnd) {
  DWORD dwStyle = WS_POPUPWINDOW | WS_OVERLAPPED | WS_BORDER |
                  WS_VISIBLE; // WS_CHILD|WS_OVERLAPPED|WS_BORDER;
  ...
  return CWnd::CreateEx(
      ...
      WS_POPUP | WS_CLIPSIBLINGS, // might want WS_BORDER on if we don't do our
                                  // own coloured border
      0, 0, 10, 10, pParentWnd ? pParentWnd->m_hWnd : NULL, NULL);
}

void CLabelThing::PopUp(LPCTSTR szLabel, POINT pt, bool bSticky,
                        bool bFrameColour, COLORREF colFrame) {
  m_strText = szLabel;
  m_ptPos = pt;
  m_bSticky = bSticky;
