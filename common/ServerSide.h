...
public:
  ...
  //				maxsize - size of largest allowable data (request
  //and 				          result sizes are both limited by this value)
  // Returns:		Success
  ...
  // ----------------------------------------------------------------------
  bool Wait() {
    return WaitForSingleObject(myRequestEvent, INFINITE) == WAIT_OBJECT_0;
  }
  ...
  //				NOTE: the request is _binary_ data rather than
  //text. 				(ie text request sizes will include the '\0' byte)
  // ----------------------------------------------------------------------
  ...
  //				both to hold the incoming request and for
  //returning 				results. The results should be simply written over the 				original
  //request. 				NOTE: the buffer is for binary data rather than text, so 				when using
  //text you'll need to cast to char*.
  // ----------------------------------------------------------------------
  unsigned char *GetBuffer() { return (unsigned char *)(mySharedMem + 1); }

  // ----------------------------------------------------------------------
  // Method:		Respond
  // Arguments:	None
  // Returns:		resultsize - size of result data in the buffer
  //				returncode - error code to return to client (0=no
  //error)
  // Description: Respond to the pending request and send the contents of
  //				the buffer back to the client.
  //				Don't forget to allow for the '\0' in the resultsize
  //if 				returning text! 				The pending request must be Respond()ed to before 				any
  ...
  // ----------------------------------------------------------------------
  unsigned int GetMaxBufferSize() { return mySharedMem->BufferSize; }
