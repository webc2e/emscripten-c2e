...
public:
  ...
  bool Connect();
  void Disconnect();
  bool Inject(std::string Macro, std::string &ReplyBuffer,
              bool bAddExecute = true);
...
private:
  ClientSide myClient;
  bool bMidInjection; // ASSERT if windows messaging should cause inject code to
                      // reenter before finishing
  bool myConnected;
  bool m_bDynConnect;
};
