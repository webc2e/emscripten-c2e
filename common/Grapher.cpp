...
void Grapher::SetWhiteColour(bool bWhiteColour) {
...
Grapher::~Grapher() { DeleteObject(m_Font); }
...
void Grapher::DoPaint(HDC pDC, const RECT &clientRect) {
  ...
  if (!(MemDC = CreateCompatibleDC(pDC)))
    return;
  if (!(OffscreenBitmap = CreateCompatibleBitmap(pDC, rect.right - rect.left,
  ...
  BitBlt(pDC, clientRect.left, clientRect.top,
...
void Grapher::DrawFrame(HDC pdc, RECT *pRect) {
  ...
  if (bAxisLabels) {
    ...
    GetTextExtentPoint(pdc, m_vLabel[0].begin(), m_vLabel[0].length(),
                       &textsize);
    {
      RECT textrect = {pRect->left, pRect->bottom - (textsize.cy * 2),
                       pRect->left + textsize.cx, pRect->bottom - textsize.cy};
      DrawText(pdc, m_vLabel[0].begin(), m_vLabel[0].length(), &textrect,
               DT_RIGHT);
    }
    {
      RECT textrect = {
          pRect->left, (pRect->top + pRect->bottom - textsize.cy * 2) / 2 - 2,
          pRect->left + textsize.cx,
          (pRect->top + pRect->bottom - textsize.cy * 2) / 2 + textsize.cy};
      DrawText(pdc, m_vLabel[1].begin(), m_vLabel[1].length(), &textrect,
               DT_RIGHT);
    }
    {
      RECT textrect = {pRect->left, pRect->top, pRect->left + textsize.cx,
                       pRect->top + textsize.cy};
      DrawText(pdc, m_vLabel[2].begin(), m_vLabel[2].length(), &textrect,
               DT_RIGHT);
    }
...
int Grapher::OnCreate() {
  ...
  if (!(m_Font = CreateFontIndirect(&logfont))) {
    MessageBox(NULL, "Could not create font for graph",
               "Graph Initialisation Error", MB_OK | MB_ICONERROR);
...
std::vector<int> Grapher::HitTestTraces(POINT point, POINT &closest,
                                        int iThreshold, float &fHitValue) {
  int i;
  int t, x, y;
  int val;
  ...
  if (PtInRect(&m_LastGraphRect, point)) // outside graph rect?
  {
    ...
    for (i = 0; i != m_vTraces.size();
         i++) // go backward to match onscreen order
    {
      if (m_vTraces[i]) {
