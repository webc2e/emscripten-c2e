...
// Class:		PrayManager
// Purpose:		To encapsulate the Pray Files and provide accessor
// functionality Description:
//  The Pray Manager manages nice lists of chunks. And allows you to find them
//  easily
//
// History:
...
// ****************************************************************************
void PrayManager::IntegrityViolation(const std::string &file,
  ...
  if (FileExists(file.c_str())) {
    GetFlightRecorder().Log(
        FLIGHT_NETBABEL, "Couldn't rename, had to try delete %s", file.c_str());
    DeleteFile(file.c_str());
  }
#endif
}
// ****************************************************************************
void PrayManager::ThrowPrayException(std::string msg, int code,
                                     std::string prayFile) {
  if (!prayFile.empty())
...
void PrayManager::GarbageCollect(bool force) {
  if (force)
    ...
    for (std::map<std::string, PrayChunkPtr>::iterator it = myChunkList.begin();
         it != myChunkList.end(); it++) {
...
void PrayManager::RescanFolders() {
  ...
  for (eit = myChunkFileExtensions.begin(); eit != myChunkFileExtensions.end();
       eit++) {
...
bool PrayManager::AddFile(std::string filename) {
  ...
  if ((p.majik[0] != 'P') || (p.majik[1] != 'R') || (p.majik[2] != 'A') ||
      (p.majik[3] != 'Y')) {
    fclose(f);
    return false;
  }
...
std::string PrayManager::GetChunkParentFile(std::string chunkName) {
  if (CheckChunk(chunkName) == 0) {
    ThrowPrayException("Chunk not found in PrayManager::GetChunkParentFile - " +
                           chunkName,
                       PrayException::eidChunkNotFound, "");
  }
...
PrayChunkPtr PrayManager::GetChunk(std::string thisChunk) {
  if (CheckChunk(thisChunk) == 0) {
    ThrowPrayException("Chunk not found in PrayManager::GetChunk - " +
                           thisChunk,
                       PrayException::eidChunkNotFound, "");
  }
  std::map<std::string, PrayChunkPtr>::iterator it;
  ...
  if ((f = fopen(locationOfChunk.first.c_str(), "rb")) == NULL) {
    ...
    ThrowPrayException("Error Opening file in PrayManager::GetChunk - " +
                           thisChunk + " @ " + locationOfChunk.first,
  ...
  if (fseek(f, locationOfChunk.second, SEEK_CUR)) {
    ...
    fclose(f);
    ThrowPrayException("File seek failed in PrayManager::GetChunk - " +
                           thisChunk + " @ " + locationOfChunk.first,
                       PrayException::eidFileTooShortInSeek,
                       locationOfChunk.first);
  }
  PrayChunkHeader ch;
  if (fread(&ch, 1, sizeof(ch), f) != sizeof(ch)) {
    ...
    fclose(f);
    ThrowPrayException(
        "File too short in PrayManager::GetChunk (Reading Chunk Header) - " +
            thisChunk + " @ " + locationOfChunk.first,
  ...
  if ((ch.flags & 1) == 0) {
    if (ch.size != ch.usize) {
      fclose(f);
      ThrowPrayException(
          "size doesn't match usize (reading uncompressed data) - " +
              thisChunk + " @ " + locationOfChunk.first,
    ...
    if (ch.size > fsize || ch.size < 0) {
      fclose(f);
      ThrowPrayException("Size told is bigger than file or is negative "
    ...
    if (fread(retVal->myPtr, 1, ch.size, f) != ch.size) {
      ...
      fclose(f);
      ThrowPrayException("File too short in PrayManager::GetChunk (Reading "
                         "uncompressed Data) - " +
                             thisChunk + " @ " + locationOfChunk.first,
  ...
  if (ch.size > fsize || ch.size < 0) {
    fclose(f);
    ThrowPrayException("Compressed size told is bigger than file or is "
                       "negative (reading compressed data) - " +
                           thisChunk + " @ " + locationOfChunk.first,
                       PrayException::eidSizeMismatch, locationOfChunk.first);
  }
  ...
  std::string uncompressCheckTag =
      "pray_uncompress_sanity_check " + FileExtension(locationOfChunk.first);
#ifndef PRAY_BUILDER
  ...
  if (fread(compressedData, 1, ch.size, f) != ch.size) {
    ...
    ThrowPrayException(
        "File to short in PrayManager::GetChunk (Reading compressed Data) - " +
            thisChunk + " @ " + locationOfChunk.first,
  ...
  if (us != ch.usize) {
    ...
    ThrowPrayException("Data size mismatch after decompress in "
                       "PrayManager::GetChunk (Checking uncompressed data) - " +
                           thisChunk + " @ " + locationOfChunk.first,
                       PrayException::eidDataMismatch, locationOfChunk.first);
  }
...
void PrayManager::GetChunks(std::string thisType,
                            std::vector<std::string> &thisVec) {
  thisVec.clear();
...
void PrayManager::AddChunkToFile(std::string thisName, std::string thisType,
                                 std::string thisFile, int thisSize,
                                 uint8 *thisData, bool doCompress) {
  // If Chunk exists, Throw
  if (CheckChunk(thisName))
    ThrowPrayException(
        "Chunk already exists in PrayManager::AddChunkToFile - " + thisName,
        PrayException::eidChunkExists, "");
  ...
  if ((f = fopen(thisFile.c_str(), "rb")) != NULL) {
    ...
    if (fread(&ph, 1, sizeof(ph), f) != sizeof(ph)) {
      ...
      ThrowPrayException(
          "File " + thisFile +
              " is _not_ a prayfile in PrayManager::AddChunkToFile",
          PrayException::eidFileTooShort, thisFile);
    }
    if ((ph.majik[0] != 'P') || (ph.majik[1] != 'R') || (ph.majik[2] != 'A') ||
        (ph.majik[3] != 'Y')) {
      // Erk - not praymajik :(
      fclose(f);
      ThrowPrayException("File " + thisFile +
                             " does not look like a prayfile to me. "
                             "(PrayManager::AddChunkToFile)",
    ...
    if ((f = fopen(thisFile.c_str(), "wb")) == NULL)
      ThrowPrayException(
          "File " + thisFile +
              " could not be opened in PrayManager::AddChunkToFile",
          PrayException::eidFileNotOpen, thisFile);
    PrayFileHeader ph;
    ph.majik[0] = 'P';
    ph.majik[1] = 'R';
    ph.majik[2] = 'A';
  ...
  if (!doCompress) {
    ...
    if (compress(cData, &len, thisData, ulen) != Z_OK) {
      ...
      fclose(f);
      ThrowPrayException(
          "Oh Pants! The compress failed!!!! (PrayManager::AddChunkToFile)",
          PrayException::eidCompressError, thisFile);
    }
...
PrayManager &PrayManager::GetPrayManager() { return ourPrayManager; }
