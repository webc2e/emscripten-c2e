...
#include "../../server/HistoryFeed/HistoryTransferOut.h"
	...
	// TODO: These shouldn't be here, might get confused with the ones in the main app
...
void NetworkImplementation::ModuleName(std::string& name)
{
	name = "netbabel";
}

int NetworkImplementation::ModuleVersion()
{
	return MINOR_VER;
}
...
void NetworkImplementation::SetDirectories(const std::string& expo, const std::string& import)
{
...
void NetworkImplementation::NotifyAgentsWhetherOnline(bool force)
{
	...
	if (force || myPreviousOnlineState != onlineNow)
	{
		...
		theAgentManager.ExecuteScriptOnAllAgents(script, COASVARAGENTNULL, INTEGERZERO, INTEGERZERO);
...
bool NetworkImplementation::Persistent()
{
	return true;
}
...
bool NetworkImplementation::HistoryFillInUserID(HistoryStore& store, const std::string& moniker)
{
...
{
public:
	HistoryQueuedMessage(const std::string& message, NetworkImplementation* networkImplementation,
	...
	{
		bool result = myNetworkImplementation->GetNetManager()->DSFeedHistory((char*)myMessage.data(), myMessage.size());
...
void NetworkImplementation::HistoryFeed(HistoryStore& store)
{
	...
	if (myHistoryResult == HISTORY_DONE_OK)
	{
		...
		if (myUploadedHistory.Count() <= history.Count())
		{
			...
			{
				std::string uploadedUserText = myUploadedHistory.GetLifeEvent(i)->myUserText;
			...
			ASSERT(false); // this should only happen if they HIST WIPE during event upload!
	...
	HistoryQueuedMessage* queueMessage =
		new HistoryQueuedMessage(std::string((char*)address, len), this, myScanningMoniker, history.myName);
...
DSNetManager* NetworkImplementation::GetNetManager()
{
	return ourNetManager;
}
